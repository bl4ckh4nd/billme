  ...
  return (
    <aside className="w-full xl:w-96 shrink-0 rounded-2xl border border-border bg-surface flex flex-col min-h-[20rem]">
      <div className="px-4 h-12 border-b border-subtle flex items-center justify-between gap-3">
        ...
        <button
          onClick={onClose}
          className="w-8 h-8 rounded-full border border-border text-muted hover:bg-surface-muted flex items-center justify-center transition-colors"
            ...
            {entries.map((entry) => (
              <div key={entry.id} className="rounded-xl border border-subtle p-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className={`text-sm font-bold whitespace-nowrap ${entry.amount >= 0 ? 'text-success' : 'text-error'}`}>
                    {euro(entry.amount)}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-2 mt-2 text-xs">
                  <div><span className="text-muted">Konto</span><div className="font-bold text-foreground">{entry.accountNumber}</div></div>
                  <div><span className="text-muted">Soll</span><div className="font-bold text-foreground">{euro(entry.debit)}</div></div>
                  <div><span className="text-muted">Haben</span><div className="font-bold text-foreground">{euro(entry.credit)}</div></div>
                </div>
                <div className="mt-2 rounded-lg bg-surface-muted px-2.5 py-2 text-[11px] text-muted" data-testid={`drilldown-source-${entry.id}`}>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {entry.sourceType === 'bank_transaction' && onOpenTransaction ? (
                      <button
                        ...
                        className="h-8 px-2.5 rounded-full border border-border text-xs font-bold text-foreground hover:bg-surface-muted transition-colors"
                      >
                        Transaktion öffnen
                      </button>
                    ) : null}
