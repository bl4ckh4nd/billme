      ...
      <div className="bg-surface-muted rounded-xl p-6 border border-border-subtle space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="px-4 py-2 bg-foreground text-white rounded-full text-xs font-bold hover:bg-dark-1 active:scale-95 transition-[background-color,border-color,color,box-shadow,transform,opacity] ease-out flex items-center gap-2"
          >
        ...
        {(settings.catalog?.categories ?? []).length === 0 ? (
          ...
          <div className="space-y-3">
            {(settings.catalog?.categories ?? []).map((cat, idx) => (
              <div key={cat.id} className="flex items-center gap-3 bg-surface rounded-xl p-3 border border-border-subtle animate-enter" style={{ animationDelay: `${idx * 50}ms` }}>
                <div className="w-10 h-10 rounded-xl bg-canvas border border-border-subtle flex items-center justify-center text-xs font-bold text-muted">
                ...
                <div className="flex-1">
                  ...
                  <input
                    ...
                    className="w-full bg-surface-muted border border-border rounded-lg p-3 text-sm font-bold outline-none focus:ring-2 focus:ring-accent transition-shadow duration-150 ease-out"
                  />
                </div>
                <button
                  onClick={() => {
                  ...
                  className="w-10 h-10 rounded-full bg-error-bg text-error hover:bg-error-bg/80 transition-colors duration-150 ease-out flex items-center justify-center"
