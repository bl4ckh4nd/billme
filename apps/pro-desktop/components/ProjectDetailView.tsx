...
import { ArrowLeft, Euro, FileText } from 'lucide-react';
    ...
    <div className="bg-surface rounded-xl p-8 min-h-full shadow-sm">
      ...
      <div className="rounded-xl border border-border bg-surface-muted p-6 mb-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
          <div className="rounded-xl bg-surface border border-border p-5 animate-scale-in" style={{ animationDelay: '0ms' }}>
          ...
          <div className="rounded-xl bg-surface border border-border p-5 animate-scale-in" style={{ animationDelay: '50ms' }}>
          ...
          <div className="rounded-xl bg-surface border border-border p-5 animate-scale-in" style={{ animationDelay: '100ms' }}>
          ...
          <div className="rounded-xl bg-surface border border-border p-5 animate-scale-in" style={{ animationDelay: '150ms' }}>
        ...
        <div className="rounded-xl border border-border overflow-hidden">
          ...
          {projectInvoices.length === 0 ? (
            <div className="p-8 text-center text-sm text-muted">
              <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-lg bg-surface-muted text-foreground border border-border">
                <FileText size={18} />
              </div>
              <p className="font-bold text-foreground">Keine Rechnungen im Projekt.</p>
              <p className="mt-1">Öffnen Sie Dokumente, um eine Rechnung diesem Projekt zuzuordnen.</p>
            </div>
          ) : (
            <div className="divide-y divide-border-subtle">
              {projectInvoices.map((d, idx) => (
                <div key={d.id} className="px-4 py-3 flex items-center justify-between animate-enter" style={{ animationDelay: `${idx * 50}ms` }}>
                  ...
                  <button
                    onClick={() => navigate({ to: '/documents' })}
                    className="px-3 py-2 rounded-xl bg-canvas hover:bg-canvas text-foreground font-bold text-sm"
                  >
                    Öffnen
                  </button>
                </div>
        ...
        <div className="rounded-xl border border-border overflow-hidden">
          ...
          {projectOffers.length === 0 ? (
            <div className="p-8 text-center text-sm text-muted">
              ...
              <p className="font-bold text-foreground">Keine Angebote im Projekt.</p>
              <p className="mt-1">Öffnen Sie Dokumente, um ein Angebot diesem Projekt zuzuordnen.</p>
            </div>
          ) : (
            <div className="divide-y divide-border-subtle">
              {projectOffers.map((d, idx) => (
                <div key={d.id} className="px-4 py-3 flex items-center justify-between animate-enter" style={{ animationDelay: `${idx * 50}ms` }}>
                  ...
                  <button
                    onClick={() => navigate({ to: '/documents' })}
                    className="px-3 py-2 rounded-xl bg-canvas hover:bg-canvas text-foreground font-bold text-sm transition-colors duration-150 ease-out"
                  >
                    Öffnen
                  </button>
                </div>
