        ...
        <div className="flex bg-canvas p-1.5 rounded-full">
            {[
                ...
                <button
                    ...
                    className={`px-6 py-2 rounded-full text-xs font-bold transition-colors duration-150 ease-out ${
                        timeRange === t.id 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-dark-3 rounded-xl p-6 text-white flex flex-col justify-between relative overflow-hidden group">
              ...
              <div className="relative z-10 flex justify-between items-start">
                  <div className="p-3 bg-surface/10 rounded-xl backdrop-blur-md">
                      <DollarSign size={20} className="text-accent" />
                  </div>
                  {revenueTrend !== null && (
                    <span className={`${revenueTrend >= 0 ? 'bg-accent/20 text-accent' : 'bg-error-bg text-error'} text-[10px] font-bold px-2 py-1 rounded-full flex items-center gap-1`}>
                        {revenueTrend >= 0 ? <TrendingUp size={10} /> : <TrendingDown size={10} />} {revenueTrend >= 0 ? '+' : ''}{revenueTrend}%
                    </span>
                  )}
              </div>
              <div className="relative z-10 mt-6">
                  <p className="text-muted text-xs font-bold uppercase tracking-wider mb-1">Umsatz (Bezahlt)</p>
          ...
          {/* Avg Ticket */}
          <div className="bg-accent rounded-xl p-6 text-foreground flex flex-col justify-between shadow-sm group">
              <div className="flex justify-between items-start">
              ...
              <div className="mt-6">
                  <p className="text-foreground/60 text-xs font-bold uppercase tracking-wider mb-1">Ø Rechnungswert</p>
              ...
              <div className="mt-6">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 bg-canvas h-2 rounded-full overflow-hidden">
                        <div 
                            className="bg-foreground h-full rounded-full transition-colors duration-1000"
                  ...
                  {chartData.map((d, i) => (
                      <div key={i} className="flex flex-col items-center gap-2 flex-1 group">
                          <div className="w-full bg-canvas rounded-t-lg relative h-full flex flex-col justify-end overflow-hidden">
                              <div 
                                className="bg-foreground w-full rounded-t-lg transition-colors duration-1000 group-hover:bg-accent relative min-h-[4px]"
                                style={{ height: `${d.height}%` }}
                              >
                                  {/* Tooltip */}
                                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-foreground text-white text-[10px] font-bold py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-10">
          ...
          <div className="bg-surface rounded-xl p-8 border border-border-subtle shadow-sm flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto pr-2 space-y-4">
                  {topClients.map((client, idx) => (
                      <div key={idx} className="flex items-center gap-4 group">
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-xs font-bold ${idx === 0 ? 'bg-accent text-foreground' : 'bg-canvas text-muted'}`}>
                              {idx + 1}
                          </div>
              ...
              <div className="mt-6 pt-6 border-t border-border-subtle">
                   <button
                       ...
                       className="w-full py-3 bg-surface-muted rounded-xl text-xs font-bold hover:bg-foreground hover:text-accent transition-colors duration-150 ease-out flex items-center justify-center gap-2"
                   >
