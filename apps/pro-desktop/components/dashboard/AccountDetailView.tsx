...
import {
  ArrowLeft, ArrowDownLeft, ArrowUpRight, FileText, Link, Search, X,
} from 'lucide-react';
import { useQueryClient } from '@tanstack/react-query';
import { Account, Invoice } from '../../types';
import { useUpsertAccountMutation } from '../../hooks/useAccounts';
...
export const AccountDetailView: React.FC<AccountDetailViewProps> = ({ account, invoices, onBack }) => {
  ...
  const refreshImportPreview = async (opts?: { path?: string; profile?: ImportProfile; mapping?: ImportMapping | null }) => {
    ...
    try {
      const res = await ipc.finance.importPreview({
        ...
        maxRows: 50,
        accountIdForDedupHash: account.id,
      });
    ...
    <div className="bg-surface rounded-xl shadow-sm overflow-hidden p-8 min-h-full flex flex-col relative animate-enter">
      ...
      {isImportOpen && (
        <div className="absolute inset-0 z-50 bg-black/20 backdrop-blur-sm flex items-center justify-center p-4 rounded-2xl animate-fade-in">
          <div className="bg-surface rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[85vh] animate-scale-in">
            <div className="p-6 border-b border-border-subtle flex justify-between items-center bg-surface-muted">
              <div>
                ...
                <p className="text-xs text-muted mt-1">Konto: {account.name}</p>
              </div>
              <button onClick={closeImport} className="p-2 hover:bg-canvas rounded-full transition-colors duration-200 ease-out"><X size={18} /></button>
            </div>
            ...
            <div className="p-6 overflow-y-auto">
              <div className="flex flex-wrap gap-3 items-end">
                <div className="flex-1 min-w-[260px]">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="px-4 py-2 rounded-lg font-bold bg-surface border border-border hover:bg-surface-muted transition-colors duration-200 ease-out text-sm"
                    >
                      Ändern
                    </button>
                  </div>
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-lg font-bold bg-foreground text-white hover:bg-dark-1 transition-colors duration-200 ease-out text-sm"
                >
                  Vorschau
                </button>
              </div>
                ...
                <>
                  ...
                  <div className="mt-6 flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="px-5 py-3 rounded-xl font-bold bg-accent text-black hover:bg-accent-hover transition-colors duration-200 ease-out"
                    >
                      Import starten
                    </button>
                  </div>
                  ...
                  {importError && (
                    <div className="mt-4 p-4 rounded-xl border border-error/30 bg-error-bg text-error font-mono text-xs">
                      {importError}
                    </div>
                  )}

                  {importResult && (
                    <div className="mt-4 p-4 rounded-xl border border-success/30 bg-success-bg text-success text-sm">
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {importPreview.stats.errorRows > 0 && (
                        <button
                          ...
                          className="px-3 py-1 rounded-lg bg-canvas hover:bg-surface-muted text-xs font-bold transition-colors duration-200 ease-out"
                        >
                        ...
                        <div className="mt-3 flex items-center justify-between text-sm">
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="px-3 py-1 rounded-lg bg-canvas hover:bg-surface-muted disabled:opacity-50 disabled:cursor-not-allowed font-bold transition-colors duration-200 ease-out"
                            >
                              Zurück
                            </button>
                            <button
      ...
      {isLinkModalOpen && (
          <div className="absolute inset-0 z-50 bg-black/20 backdrop-blur-sm flex items-center justify-center p-4 rounded-2xl animate-fade-in">
              <div className="bg-surface rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[600px] animate-scale-in">
                  <div className="p-6 border-b border-border-subtle flex justify-between items-center bg-surface-muted">
                      <h3 className="font-bold text-lg flex items-center gap-2">
                          <Link size={18} />
                          Transaktion zuweisen
                      </h3>
                      <button onClick={() => setIsLinkModalOpen(false)} className="p-2 hover:bg-canvas rounded-full transition-colors duration-200 ease-out"><X size={18} /></button>
                  </div>
                  ...
                  <div className="overflow-y-auto p-4 space-y-2">
                      {filteredInvoices.length > 0 ? filteredInvoices.map(inv => (
                          <div
                              ...
                              className="p-4 rounded-xl border border-border-subtle hover:border-foreground hover:bg-surface-muted cursor-pointer transition-[background-color,border-color,color,box-shadow,transform,opacity] duration-200 ease-out group"
                          >
      ...
      <div className="flex items-center justify-between mb-8 pb-8 border-b border-border-subtle">
          <div className="flex items-center gap-4">
              <button
                  onClick={onBack}
                  className="w-10 h-10 rounded-full border border-border flex items-center justify-center hover:bg-foreground hover:text-white transition-colors duration-200 ease-out"
              >
                  <ArrowLeft size={18} />
              </button>
              <div>
                  <h3 className="font-bold text-2xl text-foreground">{account.name}</h3>
                  <p className="text-muted font-mono text-sm">{account.iban}</p>
              </div>
          </div>
          <div className="flex items-end gap-3">
              <button
                onClick={openImportWithPicker}
                className="h-10 px-4 bg-foreground text-white rounded-full text-xs font-bold hover:bg-dark-1 transition-colors duration-200 ease-out flex items-center gap-2"
              >
                CSV importieren
              </button>
              <div className="text-right">
                  <p className="text-xs font-bold text-muted uppercase tracking-wide">Aktueller Saldo</p>
                  <p className="text-4xl tabular-nums font-bold">{formatCurrency(account.balance)}</p>
              </div>
          </div>
      </div>
          ...
          <div className="space-y-3">
              {account.transactions.map((tx, i) => {
                  ...
                  return (
                      <div
                          key={tx.id}
                          className="group flex items-center justify-between p-4 rounded-xl border border-border-subtle hover:border-border hover:shadow-md transition-[background-color,border-color,color,box-shadow,transform,opacity] duration-200 ease-out bg-surface animate-enter"
                          style={{ animationDelay: `${i * 50}ms` }}
                      >
                          <div className="flex items-center gap-4">
                              <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${tx.type === 'income' ? 'bg-accent/20 text-black' : 'bg-canvas text-muted'}`}>
                                  ...
                                  {linkedInvoice ? (
                                      ...
                                      tx.type === 'income' && (
                                          <button
                                              ...
                                              className="flex items-center gap-1.5 bg-foreground text-accent px-3 py-1 rounded text-[10px] font-bold hover:bg-dark-1 transition-colors duration-200 ease-out opacity-0 group-hover:opacity-100 transform translate-y-1 group-hover:translate-y-0"
                                          >
                                              <Link size={10} />
                                              Zuweisen
                                          </button>
              ...
              {account.transactions.length === 0 && (
                  <div className="text-center py-12 text-muted">
                      ...
                      <p className="text-xs mt-1">Importieren Sie eine CSV-Datei, um zu beginnen.</p>
                  </div>
              )}
          </div>
      </div>
