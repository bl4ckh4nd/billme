...
const backfillOutgoingSourceVersion = (row: any, documentVersion: string, reservation: FinalizedInvoiceReservation | null): string =>
  hash({ documentVersion, eligibility: { status: row.status, reservation: reservation && { id: reservation.id, kind: reservation.kind, number: reservation.number, status: reservation.status, documentId: reservation.document_id } } });
