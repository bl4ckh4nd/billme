...
export const ClientListRow: React.FC<ClientListRowProps> = ({ client, idx, onClick }) => (
    <div
        key={client.id}
        onClick={onClick}
        className="group bg-surface-muted rounded-xl p-4 border border-border-subtle hover:border-border hover:bg-surface transition-[background-color,border-color,color,box-shadow,opacity,transform,width] ease-out duration-150 grid grid-cols-12 gap-4 items-center cursor-pointer animate-enter"
        style={{ animationDelay: `${idx * 50}ms` }}
    >
        <div className="col-span-4 flex items-center gap-4">
            <div className="w-10 h-10 bg-surface rounded-lg flex items-center justify-center text-sm font-bold shadow-sm text-foreground shrink-0">
        ...
        <div className="col-span-1 flex justify-end opacity-0 group-hover:opacity-100 transition-opacity ease-out duration-150">
