...
const numberInputClassName = 'min-w-0 w-full bg-surface-muted rounded-lg px-1.5 py-1 text-sm tabular-nums text-right focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus-ring';
const inputClassName = 'min-w-0 w-full bg-surface-muted rounded-lg px-1.5 py-1 text-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus-ring';
const miniLabelClassName = 'block whitespace-normal leading-tight text-xs font-medium text-muted mb-0.5';
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5">
        ...
        <span className="w-5 shrink-0 text-xs font-bold text-muted tabular-nums text-right">{index + 1}</span>
        <div className="flex-1 min-w-0" data-field-error={descriptionError ? true : undefined}>
          <Combobox
            ...
            inputClassName={`bg-surface-muted border hover:border-control-border focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus-ring rounded-lg px-2 py-1 text-sm font-bold ${descriptionError ? 'border-error-text' : 'border-transparent'}`}
          />
          {descriptionError ? <p className="mt-1 text-xs text-error-text">{descriptionError}</p> : null}
        </div>
        <div className="flex items-center gap-0.5 motion-safe:transition-opacity motion-reduce:transition-none md:opacity-0 md:group-hover:opacity-100 md:focus-within:opacity-100">
          <button
            type="button"
      ...
      <div className="grid grid-cols-2 gap-2 pl-[26px] items-end sm:grid-cols-3">
        ...
        <div className="min-w-0 pb-1">
          ...
          {item.discountPercent ? <div className="text-xs text-muted line-through tabular-nums text-right">{formatCurrency(baseTotal)}</div> : null}
