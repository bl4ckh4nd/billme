        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            ...
            <div className="rounded-xl bg-warning-bg border border-warning-border p-3">
              ...
              <div className="text-lg font-mono font-bold text-warning mt-1">{report.unclassifiedCount}</div>
            </div>
          </div>
          ...
          {report.rows.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-muted">
              ...
              <p className="text-sm font-medium">Noch keine Zeilen</p>
              <p className="text-xs text-center mt-1">Klassifizieren Sie Einträge, um den Report zu befüllen.</p>
            </div>
          ) : (
            <div className="max-h-[470px] overflow-auto">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {report.rows.map((row) => (
                    <tr key={row.lineId} className="border-b border-border-subtle hover:bg-surface-muted transition-colors ease-out duration-150">
