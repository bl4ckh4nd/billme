...
export const createLiteHttpBillmeApi = ({
  ...
  const requestJson = async <TSchema extends z.ZodTypeAny>(
    ...
    const response = await fetchImpl(`${normalizeBaseUrl(connection.baseUrl)}${path}`, {
      method,
      headers: {
        ...authHeaders,
  ...
  const invoke = async <K extends IpcRouteKey>(key: K, rawArgs: IpcArgs<K>): Promise<IpcResult<K>> => {
    ...
    switch (key) {
      ...
      case 'eur:getReport': {
        ...
        return mapEurReport(await requestJson('GET', `${PRODUCT_PREFIX}/reports/eur`, serverEurReportSchema, { from: parsed.from ?? '2025-01-01', to: parsed.to ?? '2025-12-31' })) as IpcResult<K>;
      }
      case 'eur:listItems': {
        const parsed = args as IpcArgs<'eur:listItems'>;
        if (parsed.taxYear !== 2025) throw new Error('EÜR unterstützt ausschließlich das Steuerjahr 2025.');
        const items = await requestJson('GET', `${PRODUCT_PREFIX}/reports/eur/items`, z.array(serverEurItemSchema), { from: parsed.from ?? '2025-01-01', to: parsed.to ?? '2025-12-31' });
      ...
      case 'eur:exportCsv': {
        ...
        if (parsed.taxYear !== 2025) throw new Error('EÜR unterstützt ausschließlich das Steuerjahr 2025.');
        const report = mapEurReport(await requestJson('GET', `${PRODUCT_PREFIX}/reports/eur`, serverEurReportSchema, { from: parsed.from ?? '2025-01-01', to: parsed.to ?? '2025-12-31' }));
