  ...
  app.setErrorHandler((error, request, reply) => {
    ...
    if (error instanceof Error) {
      ...
      const status =
        ...
        /^(POSTING_DATE_IN_CLOSED_PERIOD|BACKFILL_STALE_PREVIEW|BACKFILL_CONFIRMATION_HASH_MISMATCH|DRAFT_NOT_APPROVED|DOCUMENT_HAS_ALLOCATIONS|DOCUMENT_NOT_POSTED|VENDOR_HAS_POSTED_DOCUMENTS|PERIOD_LOCKED|SOFT_LOCK_|FINALIZED_|ALREADY_|.*_POSTED$|.*_CLOSED$)/.test(code) ? 409 :
        /^(UNBALANCED_|TAX_|VALIDATION_|INVALID_JOURNAL_|INVALID_LINE_|INVALID_TAX_|AMBIGUOUS_TAX_|UNKNOWN_ACCOUNT)/.test(code) ? 422 :
        /^(INVALID_|MISSING_|UNKNOWN_|EMPTY_|PAYMENT_|OPEN_ITEM_|DRAFT_|BANK_TRANSACTION_|BACKFILL_)/.test(code) ? 400 :
