    ...
    return (
        <div className="bg-surface rounded-xl p-8 min-h-full shadow-sm flex flex-col animate-enter relative">
            <div className="flex items-center justify-between mb-8">
                ...
                <button 
                    onClick={() => handleEdit()}
                    className="bg-accent text-foreground px-6 py-3 rounded-full font-bold hover:scale-105 active:scale-95 transition-colors duration-150 ease-out shadow-lg shadow-accent/20 flex items-center gap-2"
                >
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 overflow-y-auto pb-4">
                {profiles.map((profile, idx) => (
                    <div 
                        key={profile.id}
                        className={`p-6 rounded-xl border transition-colors duration-150 ease-out relative overflow-hidden group animate-scale-in ${profile.active ? 'bg-surface border-border hover:border-foreground hover:shadow-xl' : 'bg-surface-muted border-border-subtle opacity-70'}`}
                    ...
                    >
                        <div className="flex justify-between items-start mb-6">
                            ...
                            <div className="flex items-center gap-2">
                                <button 
                                    ...
                                    className={`px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1 transition-colors duration-150 ease-out ${profile.active ? 'bg-success-bg text-success' : 'bg-canvas text-muted'}`}
                                >
                                ...
                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => handleEdit(profile)} className="p-2 bg-canvas hover:bg-foreground hover:text-white rounded-lg transition-colors duration-150 ease-out"><Edit3 size={14}/></button>
                                    <button onClick={() => handleDelete(profile.id)} className="p-2 bg-error-bg text-error hover:bg-error hover:text-white rounded-lg transition-colors duration-150 ease-out"><Trash2 size={14}/></button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between pt-4 border-t border-border-subtle">
                             ...
                             <button
                                ...
                                className="text-xs font-bold text-foreground hover:text-accent hover:bg-foreground px-3 py-1.5 rounded-lg transition-colors duration-150 ease-out border border-transparent hover:border-foreground disabled:opacity-50 disabled:cursor-not-allowed"
                             >
            ...
            {/* Edit Modal */}
            {isEditModalOpen && (
                <div className="absolute inset-0 z-50 bg-foreground/50 backdrop-blur-sm flex justify-end">
                    <div className="w-full max-w-2xl bg-surface h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300 rounded-l-[2.5rem] overflow-hidden">
                        <div className="p-8 border-b border-border-subtle flex justify-between items-center bg-surface-muted">
                            ...
                            <button onClick={() => setIsEditModalOpen(false)} className="p-2 hover:bg-canvas rounded-full transition-colors duration-150 ease-out"><X size={20} /></button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-8 space-y-8">
                            ...
                            <section className="space-y-4">
                                <div className="flex justify-between items-center">
                                    ...
                                    <button onClick={addItem} className="text-xs font-bold bg-foreground text-accent px-2 py-1 rounded hover:bg-dark-1 transition-colors duration-150 ease-out">
                                        + Position
                                    </button>
                                </div>
                                        ...
                                        <div key={idx} className="bg-surface-muted rounded-xl p-3 border border-border-subtle">
                                            ...
                                            <div className="grid grid-cols-3 gap-2">
                                                <div>
                                                    ...
                                                    <input 
                                                        type="number" 
                                                        className="w-full bg-surface border border-border rounded p-2 text-sm outline-none"
                                                        value={item.quantity}
                        ...
                        <div className="p-6 border-t border-border-subtle bg-surface-muted flex justify-end gap-3">
                            <button onClick={() => setIsEditModalOpen(false)} className="px-6 py-3 rounded-xl font-bold text-muted hover:bg-canvas transition-colors duration-150 ease-out">Abbrechen</button>
                            <button 
                                onClick={handleSave}
                                disabled={!formData.name}
                                className="px-6 py-3 rounded-xl font-bold bg-accent text-foreground hover:bg-accent-hover shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                            >
