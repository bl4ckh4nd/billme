...
interface DocumentPreviewItem extends InvoiceItem {
  total: number;
}
...
interface DocumentPreviewProps {
  ...
  selectedDocumentItems: DocumentPreviewItem[];
  selectedDocumentNet: number;
  selectedDocumentVatRate: number;
  selectedDocumentVat: number;
  selectedDocumentGross: number;
...
export const DocumentPreview: React.FC<DocumentPreviewProps> = ({
  ...
  selectedDocumentNet,
  selectedDocumentVatRate,
  selectedDocumentVat,
  selectedDocumentGross,
       ...
       <div className="flex flex-col md:flex-row justify-between gap-8 mb-10 pb-8 border-b border-border border-dashed">
           ...
           <div className="flex gap-8">
                <div>
                    ...
                    <p className="font-bold text-foreground">{formatDate(selectedDocument.date)}</p>
                </div>
                <div>
                    <p className="text-[10px] font-bold text-muted uppercase tracking-widest mb-2 flex items-center gap-1">
                        <Calendar size={12}/> Leistungsdatum
                    </p>
                    <p className="font-bold text-foreground">{formatDate(selectedDocument.servicePeriod || selectedDocument.date)}</p>
                </div>
                <div>
                    <p className="text-[10px] font-bold text-muted uppercase tracking-widest mb-2 flex items-center gap-1">
                        <Clock size={12}/> {documentType === 'offer' ? 'Gültig bis' : 'Fällig'}
                   </p>
                   <p className={`font-bold ${selectedDocument.status === 'overdue' ? 'text-error' : 'text-foreground'}`}>
               ...
               <tbody className="divide-y divide-border/50">
                   {selectedDocumentItems.map((item, i) => (
                       <tr key={i} className="group hover:bg-surface/50 transition-colors ease-out duration-150">
                           ...
                           <td className="py-4 text-right text-muted text-sm">{item.quantity}</td>
                           <td className="py-4 text-right text-muted text-sm">{formatCurrency(item.price)}</td>
                           <td className="py-4 text-right font-bold text-foreground pr-2">{formatCurrency(item.total)}</td>
                       </tr>
                   ))}
               </tbody>
           </table>
            ...
            <div className="flex-1">
             ...
             <div className="w-full md:w-64 space-y-2">
                 <div className="flex justify-between text-sm text-muted">
                     <span>Netto</span>
                     <span className="tabular-nums">{formatCurrency(selectedDocumentNet)}</span>
                 </div>
                 <div className="flex justify-between text-sm text-muted">
                     <span>MwSt {selectedDocumentVatRate}%</span>
                     <span className="tabular-nums">{formatCurrency(selectedDocumentVat)}</span>
                 </div>
                 <div className="flex justify-between text-xl font-bold text-foreground border-t border-border pt-3 mt-1">
                     <span>Gesamt</span>
                     <span className="tabular-nums">{formatCurrency(selectedDocumentGross || selectedDocument.amount)}</span>
                 </div>
             </div>
       </div>
    </div>
