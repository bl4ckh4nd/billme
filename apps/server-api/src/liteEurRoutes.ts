...
export const registerLiteEurRoutes = (app: FastifyInstance): void => {
    ...
    async handler({ request, body }) {
      ...
      try {
        const input = {
          ...
          mutation: {
            reason: body.reason,
            actor: { type: 'user' as const, id: session.user.id, displayName: session.user.fullName },
          },
        };
        return body.taxYear === 2026
          ? await saveServerEurClassificationFact(requirePool(app), session.scope, input)
          : await createPostgresProAccountingRepository(requirePool(app)).upsertEurClassification(session.scope, input);
      } catch (error) {
        return mapEurError(error);
      }
    },
  ...
  typedRoute(app, { method: 'POST', url: `${prefix}/facts/cash`, body: liteEurCashFactBodySchema, async handler({ request, body }) {
    const session = await requireSession(app, 'lite', request.headers.authorization);
    if (!isMutationRole(session.role)) throw new ApiError(403, 'EÜR-Fakten benötigen die Rolle owner, admin oder accountant.');
    try {
      return await saveServerEurCashFact(requirePool(app), session.scope, { ...body, product: 'lite', mutation: { reason: body.reason, actor: { type: 'user', id: session.user.id, displayName: session.user.fullName } } });
    } catch (error) {
      return mapEurError(error);
    }
  }});
  typedRoute(app, { method: 'POST', url: `${prefix}/facts/annex`, body: liteEurAnnexFactBodySchema, async handler({ request, body }) {
    ...
    if (!isMutationRole(session.role)) throw new ApiError(403, 'EÜR-Fakten benötigen die Rolle owner, admin oder accountant.');
    try {
      return await saveServerEurAnnexFact(requirePool(app), session.scope, { ...body, mutation: { reason: body.reason, actor: { type: 'user', id: session.user.id, displayName: session.user.fullName } } });
    } catch (error) {
      return mapEurError(error);
    }
  }});
