    ...
    <div className="flex flex-col gap-6 h-full pb-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-surface rounded-xl p-6 shadow-sm">
        <div>
           <h1 className="text-3xl font-black text-foreground flex items-center gap-3">
               <BarChart3 className="text-foreground" />
               Statistiken
           </h1>
        ...
        <div className="flex bg-canvas p-1.5 rounded-full">
            {[
                ...
                <button
                    ...
                    className={`px-6 py-2 rounded-full text-xs font-bold transition-[background-color,border-color,color,box-shadow,opacity,transform,width] ease-out ${
                        timeRange === t.id
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-dark-3 rounded-xl p-6 text-white flex flex-col justify-between relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-32 h-32 bg-accent rounded-full blur-[60px] opacity-10 group-hover:opacity-20 transition-opacity ease-out"></div>
              <div className="relative z-10 flex justify-between items-start">
                  <div className="p-3 bg-white/10 rounded-xl backdrop-blur-md">
                      <DollarSign size={20} className="text-accent" />
                  </div>
                  {revenueTrend !== null && (
                    <span className={`${revenueTrend >= 0 ? 'bg-accent/20 text-accent' : 'bg-error/20 text-error'} text-[10px] font-bold px-2 py-1 rounded-full flex items-center gap-1`}>
                        {revenueTrend >= 0 ? <TrendingUp size={10} /> : <TrendingDown size={10} />} {revenueTrend >= 0 ? '+' : ''}{revenueTrend}%
                    </span>
                  )}
              </div>
              <div className="relative z-10 mt-6">
                  <p className="text-dark-muted text-xs font-bold uppercase tracking-wider mb-1">Umsatz (Bezahlt)</p>
                  <h3 className="text-3xl tabular-nums font-bold">{formatCurrency(kpis.revenue)}</h3>
              </div>
          </div>

          {/* Outstanding */}
          <div className="bg-surface rounded-xl p-6 border border-border-subtle flex flex-col justify-between">
              ...
              <div className="mt-6">
                  ...
                  <div className="flex items-end gap-2">
                      <h3 className="text-3xl tabular-nums font-bold text-foreground">{formatCurrency(kpis.outstanding)}</h3>
                      {kpis.overdue > 0 && (
          ...
          <div className="bg-accent rounded-xl p-6 text-black flex flex-col justify-between shadow-sm group">
              ...
              <div className="mt-6">
                  ...
                  <h3 className="text-3xl tabular-nums font-bold">{formatCurrency(kpis.avgTicket)}</h3>
              </div>
          </div>
           ...
           <div className="bg-surface rounded-xl p-6 border border-border-subtle flex flex-col justify-between">
              ...
              <div className="mt-6">
                  <p className="text-muted text-xs font-bold uppercase tracking-wider mb-1">Zahlungsquote</p>
                  <div className="flex items-center gap-3">
                    <h3 className="text-3xl tabular-nums font-bold text-foreground">{Math.round(kpis.conversionRate)}%</h3>
                    <div className="flex-1 bg-canvas h-2 rounded-full overflow-hidden">
                        <div
                            className="bg-foreground h-full rounded-full transition-[background-color,border-color,color,box-shadow,opacity,transform,width] duration-700 ease-out"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-[400px]">
          ...
          {/* Main Chart */}
          <div className="lg:col-span-2 bg-surface rounded-xl p-8 border border-border-subtle shadow-sm flex flex-col">
              ...
              <div className="flex-1 flex items-end justify-between gap-2 md:gap-4 min-h-[250px] pb-4 px-2">
                  {chartData.map((d, i) => (
                      <div key={i} className="flex flex-col items-center gap-2 flex-1 group">
                          <div className="w-full bg-canvas rounded-t-lg relative h-full flex flex-col justify-end overflow-hidden">
                              <div
                                className="bg-foreground w-full rounded-t-lg transition-[background-color,border-color,color,box-shadow,opacity,transform,width] duration-700 ease-out group-hover:bg-accent relative min-h-[4px]"
                                style={{ height: `${d.height}%` }}
                              >
                                  {/* Tooltip */}
                                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-foreground text-background text-[10px] font-bold py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity ease-out pointer-events-none whitespace-nowrap z-10">
                                      {formatCurrency(d.value)}
                                  </div>
                              </div>
                          </div>
                          <span className="text-[10px] font-bold text-muted uppercase">{d.label}</span>
                      </div>
                  ))}
              </div>
          </div>
          ...
          {/* Top Customers List */}
          <div className="bg-surface rounded-xl p-8 border border-border-subtle shadow-sm flex flex-col">
              <h3 className="font-bold text-xl mb-6">Top Kunden</h3>
              <div className="flex-1 overflow-y-auto pr-2 space-y-4">
                  {topClients.map((client, idx) => (
                      <div key={idx} className="flex items-center gap-4 group">
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-xs font-bold ${idx === 0 ? 'bg-accent text-accent-foreground' : 'bg-canvas text-muted'}`}>
                              {idx + 1}
                          </div>
                          <div className="flex-1">
                              <div className="flex justify-between items-center mb-1">
                                  <span className="font-bold text-sm text-foreground">{client.name}</span>
                                  <span className="tabular-nums font-bold text-sm">{formatCurrency(client.amount)}</span>
                              </div>
              ...
              <div className="mt-6 pt-6 border-t border-border-subtle">
                   <button
                       ...
                       className="w-full py-3 bg-surface-muted rounded-xl text-xs font-bold hover:bg-foreground hover:text-accent transition-colors ease-out flex items-center justify-center gap-2"
                   >
