...
export type ProAccountingReportRepository = ProAccountingRepository & ProAccountingAssetRepository & {
  listReportSnapshots(scope: TenantScope, reportType?: string): Promise<Array<{ id: string; reportType: string; args: unknown; payload: unknown; createdAt: string }>>;
  getReportSnapshot(scope: TenantScope, id: string): Promise<{ id: string; reportType: string; args: unknown; payload: unknown; createdAt: string }>;
  saveReportSnapshot(scope: TenantScope, input: { reportType: string; args: unknown; payload: unknown; mutation?: AccountingMutationContext }): Promise<{ id: string; reportType: string; args: unknown; payload: unknown; createdAt: string }>;
...
export const createPostgresProAccountingRepository = (db: PostgresQueryable): ProAccountingReportRepository => ({
  ...
  async listReportSnapshots(scope, reportType) { const rows = await q<any>(db, `SELECT id,report_type,args_json,payload_json,created_at FROM report_snapshots WHERE tenant_id=$1${reportType ? ' AND report_type=$2' : ''} ORDER BY created_at DESC,id DESC`, reportType ? [tenant(scope), reportType] : [tenant(scope)]); return rows.map((r) => ({ id: r.id, reportType: r.report_type, args: parse(r.args_json, {}), payload: parse(r.payload_json, null), createdAt: r.created_at })); },
  async getReportSnapshot(scope, id) { const row = (await q<any>(db, `SELECT id,report_type,args_json,payload_json,created_at FROM report_snapshots WHERE tenant_id=$1 AND id=$2`, [tenant(scope), id]))[0]; if (!row) throw new Error('REPORT_SNAPSHOT_NOT_FOUND'); return { id: row.id, reportType: row.report_type, args: parse(row.args_json, {}), payload: parse(row.payload_json, null), createdAt: row.created_at }; },
  async saveReportSnapshot(scope, input) { return inTx(db, async (tx) => { const id = randomUUID(); const createdAt = now(); await q(tx, `INSERT INTO report_snapshots (id,tenant_id,report_type,args_json,payload_json,created_at) VALUES ($1,$2,$3,$4,$5,$6)`, [id, tenant(scope), input.reportType, JSON.stringify(input.args ?? {}), JSON.stringify(input.payload), createdAt]); await audit(tx as PostgresTransactionClient, scope, 'report_snapshot', id, 'create', input.mutation?.reason ?? 'Report snapshot created', null, { reportType: input.reportType, args: input.args }, input.mutation); return { id, reportType: input.reportType, args: input.args ?? {}, payload: input.payload, createdAt }; }); },
  async getAccountMappingHealth(scope, chart) { const p = await policy(db, tenant(scope)); const activeChart = chart ?? p.activeChart; const rows = await q<any>(db, `SELECT DISTINCT jl.account_number,required.statement_type FROM journal_lines jl CROSS JOIN (VALUES ('guv'::text),('bilanz'::text)) required(statement_type) LEFT JOIN account_mappings_hgb am ON am.tenant_id=jl.tenant_id AND am.chart=$2 AND am.account_number=jl.account_number AND am.statement_type=required.statement_type WHERE jl.tenant_id=$1 AND am.id IS NULL ORDER BY jl.account_number,required.statement_type`, [tenant(scope), activeChart]); return { chart: activeChart, unmapped: rows.map((r) => ({ accountNumber: String(r.account_number), statementType: String(r.statement_type) })) }; },
  async upsertAccountMappingOverride(scope, input) { return inTx(db, async (tx) => { const id = randomUUID(); const stamp = now(); await q(tx, `INSERT INTO account_mappings_hgb (id,tenant_id,chart,account_number,statement_type,position_key,position_label,balance_side,updated_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT (tenant_id,chart,account_number,statement_type) DO UPDATE SET position_key=EXCLUDED.position_key,position_label=EXCLUDED.position_label,balance_side=EXCLUDED.balance_side,updated_at=EXCLUDED.updated_at`, [id, tenant(scope), input.chart, input.accountNumber.trim(), input.statementType, input.positionKey.trim(), input.positionLabel.trim(), input.balanceSide ?? null, stamp]); await audit(tx as PostgresTransactionClient, scope, 'account_mapping_hgb', `${input.chart}:${input.statementType}:${input.accountNumber}`, 'override', input.mutation?.reason ?? 'Report mapping override', null, input, input.mutation); const row = (await q<any>(tx, `SELECT id,tenant_id,chart,account_number,statement_type,position_key,position_label,balance_side,updated_at FROM account_mappings_hgb WHERE tenant_id=$1 AND chart=$2 AND account_number=$3 AND statement_type=$4`, [tenant(scope), input.chart, input.accountNumber.trim(), input.statementType]))[0]; return { id: row.id, tenantId: row.tenant_id, chart: row.chart, accountNumber: row.account_number, statementType: row.statement_type, positionKey: row.position_key, positionLabel: row.position_label, balanceSide: row.balance_side ?? undefined, updatedAt: row.updated_at }; }); },
 ...
 await ensurePeriod(tx, t, period(stamp.slice(0, 10))); }); },
});
