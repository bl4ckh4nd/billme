    ...
    <aside className="w-full xl:w-96 shrink-0 rounded-2xl border border-border bg-surface flex flex-col min-h-[20rem]">
      <div className="px-4 h-12 border-b border-subtle flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="text-xs font-semibold text-muted">{REPORT_TYPE_LABEL[selection.reportType]}</div>
          <div className="text-sm font-semibold text-foreground truncate">{selection.targetLabel}</div>
        </div>
        <button
          onClick={onClose}
            ...
            {entries.map((entry) => (
              <div key={entry.id} className="rounded-xl border border-subtle p-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <div className="text-sm font-semibold text-foreground">{entry.bookingText}</div>
                    <div className="text-xs text-muted mt-0.5">
                      {new Date(entry.date).toLocaleDateString('de-DE')} • {formatEmptyValue(entry.reference)} • {entry.source}
                    </div>
                  </div>
                  <div className={`text-sm font-semibold tabular-nums whitespace-nowrap ${entry.amount >= 0 ? 'text-success-text' : 'text-error-text'}`}>
                    {euro(entry.amount)}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-2 mt-2 text-xs">
                  <div><span className="text-muted">Konto</span><div className="font-semibold text-foreground">{entry.accountNumber}</div></div>
                  <div><span className="text-muted">Soll</span><div className="font-semibold tabular-nums text-foreground">{euro(entry.debit)}</div></div>
                  <div><span className="text-muted">Haben</span><div className="font-semibold tabular-nums text-foreground">{euro(entry.credit)}</div></div>
                </div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {entry.sourceType === 'bank_transaction' && onOpenTransaction ? (
                      <button
                        ...
                        className="h-8 px-2.5 rounded-lg border border-control-border text-xs font-semibold text-foreground hover:bg-surface-muted transition-colors"
                      >
                        Transaktion öffnen
                      </button>
                    ) : null}
