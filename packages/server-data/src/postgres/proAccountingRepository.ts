...
export const createPostgresProAccountingRepository = (db: PostgresQueryable): ProAccountingRepository => ({
  ...
  async insertDatevExport(scope, args) { return inTx(db, async (tx) => { const id = randomUUID(); const createdAt = now(); await q(tx, `INSERT INTO datev_exports (id,tenant_id,file_path,record_count,from_date,to_date,created_at,meta_json) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`, [id, tenant(scope), args.filePath, args.recordCount, args.fromDate ?? null, args.toDate ?? null, createdAt, JSON.stringify({ immutable: true })]); await audit(tx as PostgresTransactionClient, scope, 'pro_datev_export', id, 'export', 'DATEV export recorded', null, args); return { id, ...args, createdAt }; }); },
