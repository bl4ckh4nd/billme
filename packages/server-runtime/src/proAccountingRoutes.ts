...
import {
  ...
  openRouterVlmPublicConfigSchema,
  OpenRouterVlmError,
  type OpenRouterVlmService,
...
import { createMutationContext, requireDatabase, requireMutationSession, requireSession } from './runtimeContext.js';
...
const mutationFor = (session: Awaited<ReturnType<typeof requireProSession>>, reason: string): AccountingMutationContext => createMutationContext(session, reason);
...
export interface ProAccountingRouteOptions {
  openRouterVlmService?: OpenRouterVlmService;
}

export const registerProAccountingRoutes = (app: FastifyInstance, options: ProAccountingRouteOptions = {}) => {
  const prefix = '/api/v1/pro/accounting';
  const openRouterVlm = options.openRouterVlmService ?? createOpenRouterVlmService();

  typedRoute(app, {
    method: 'GET',
    url: `${prefix}/vlm/config`,
    response: openRouterVlmPublicConfigSchema,
    async handler({ request }) {
      await requireProSession(app, request.headers.authorization);
      return openRouterVlm.getConfig();
    },
  });

  const analyzeTransactionDocument = async ({ request, params, body }: { request: FastifyRequest; params: z.infer<typeof transactionParams>; body: z.infer<typeof openRouterVlmAnalyzeInputSchema> }) => {
    const session = await requireProSession(app, request.headers.authorization);
    if (body.transaction.id !== params.transactionId) {
      throw new ApiError(400, 'transaction.id muss mit der URL übereinstimmen.');
    }
    const transaction = (await serviceFor(app).listBankTransactions(session.scope)).find((candidate) => candidate.id === params.transactionId);
    if (!transaction) throw new ApiError(404, 'Transaktion wurde nicht gefunden.');
    try {
      // Transaction facts are read from the tenant-scoped accounting store;
      // browser values are only a display hint and must not influence matching.
      return await openRouterVlm.analyze({
        ...body,
        transaction: {
          id: transaction.id,
          ...
          ...(transaction.linkedInvoiceId ? { linkedInvoiceId: transaction.linkedInvoiceId } : {}),
          ...(transaction.suggestedAccountNumber ? { suggestedAccountNumber: transaction.suggestedAccountNumber } : {}),
          ...(transaction.suggestionReason ? { suggestionReason: transaction.suggestionReason } : {}),
        },
      });
    } catch (error) {
      if (error instanceof OpenRouterVlmError) {
        const status = error.code === 'OPENROUTER_NOT_CONFIGURED' ? 503 : error.code === 'OPENROUTER_REQUEST_FAILED' || error.code === 'OPENROUTER_TIMEOUT' ? 502 : 400;
  ...
  typedRoute(app, {
    method: 'POST',
    url: `${prefix}/transactions/:transactionId/analyze-document`,
    params: transactionParams,
    body: openRouterVlmAnalyzeInputSchema,
    response: openRouterVlmAnalyzeResultSchema,
    handler: analyzeTransactionDocument,
  });
  ...
  typedRoute(app, {
    ...
    async handler({ request, body }) {
      ...
      try {
        ...
        if (error instanceof Error && /IDEMPOTENCY|SOURCE_RUN_CONFLICT|OVER_CREDIT|POSTING_DATE_IN_CLOSED_PERIOD|SOFT_LOCK|UNKNOWN_ACCOUNT|OPOS|ORIGINAL_CHANGED|DOCUMENT_NOT_POSTED|OPEN_ITEM_NOT_ELIGIBLE|OPEN_ITEM_NOT_POSTED|SETTLEMENT_EXCEEDS_OPEN_ITEM|SETTLEMENT_PARTY_MISMATCH/.test(error.message)) throw new ApiError(409, error.message);
        if (error instanceof Error && /INVALID|REQUIRED|ORIGINAL_DOCUMENT_NOT_FOUND|ORIGINAL_SNAPSHOT_REQUIRED|OPEN_ITEM_NOT_FOUND|TAX_EFFECTIVE/.test(error.message)) throw new ApiError(400, error.message);
        throw error;
      }
    },
  });
  ...
  typedRoute(app, {
    ...
    async handler({ request, body }) {
      ...
      try {
        ...
        if (error instanceof Error && /SOURCE_RUN_CONFLICT|POSTING_DATE_IN_CLOSED_PERIOD|SOFT_LOCK|UNKNOWN_ACCOUNT|FISCAL_YEAR_PERIOD_MISMATCH|AGGREGATE|SETTLEMENT_EXCEEDS_OPEN_ITEM|SETTLEMENT_PARTY_MISMATCH|OPEN_ITEM_NOT_ELIGIBLE|OPEN_ITEM_NOT_POSTED|DOCUMENT_NOT_POSTED/.test(error.message)) throw new ApiError(409, error.message);
        if (error instanceof Error && /INVALID|REJECTED|REQUIRED|MISSING|OPEN_ITEM_NOT_FOUND/.test(error.message)) throw new ApiError(400, error.message);
        throw error;
      }
    },
  });
    ...
    async handler({ request, query }) {
      ...
      try {
        const items = query.taxYear === 2026
          ? await listServerEurCashItems(requireDatabase(app), session.scope, query)
  ...
  typedRoute(app, {
    ...
    async handler({ request, body }) {
      const session = await requireMutationSession(app, request.headers.authorization);
      try {
        return await saveServerEurCashFact(requireDatabase(app), session.scope, { ...body, mutation: mutationFor(session, body.reason) });
      } catch (error) {
  ...
  typedRoute(app, {
    ...
    async handler({ request, body }) {
      const session = await requireMutationSession(app, request.headers.authorization);
      try {
        return await saveServerEurAnnexFact(requireDatabase(app), session.scope, { ...body, mutation: mutationFor(session, body.reason) });
      } catch (error) {
        return mapEurRuntimeError(error);
      }
    },
    ...
    async handler({ request, query }) {
      ...
      try {
        return query.taxYear === 2026
          ? await getServerEurReport(requireDatabase(app), session.scope, query)
  ...
  typedRoute(app, {
    ...
    async handler({ request, reply, query }) {
      const session = await requireProSession(app, request.headers.authorization);
      try {
        const report = await getServerEurReport(requireDatabase(app), session.scope, query);
  ...
  typedRoute(app, {
    ...
    async handler({ request, query }) {
      const session = await requireProSession(app, request.headers.authorization);
      if (query.reportType === 'eur' && query.taxYear === 2026) return listServerEurSnapshots(requireDatabase(app), session.scope, 2026);
  ...
  typedRoute(app, {
    ...
    async handler({ request, params }) {
      const session = await requireProSession(app, request.headers.authorization);
      const eurSnapshot = await getServerEurSnapshot(requireDatabase(app), session.scope, params.id);
  ...
  typedRoute(app, {
    ...
    async handler({ request, body }) {
      ...
      if (body.reportType === 'eur' && body.taxYear === 2026) {
        const payload = await getServerEurReport(requireDatabase(app), session.scope, { taxYear: 2026, from: body.from, to: body.to });
        return freezeServerEurSnapshot(requireDatabase(app), session.scope, payload, mutationFor(session, body.reason));
      }
