...
const actorFor = (session: AuthSession): AuditActor => ({
  type: 'user',
  id: session.user.id,
  displayName: session.user.fullName,
});

const requireAutomationMutationSession = async (
  sessionFor: (authorization: string | undefined) => Promise<AuthSession>,
  authorization: string | undefined,
): Promise<AuthSession> => {
  const session = await sessionFor(authorization);
  if (!['owner', 'admin', 'accountant'].includes(session.role)) {
    throw new ApiError(403, 'Diese Aktion benötigt die Rolle owner, admin oder accountant.');
  }
  return session;
};
...
const queryTarget = (database: RuntimeQueryTarget): PostgresQueryable => database as PostgresQueryable;

const withRuntimeTransaction = async <T>(
  database: RuntimeDatabase,
  work: (session: PostgresQueryable) => Promise<T>,
): Promise<T> => {
  if ('engine' in database) {
    return database.transaction({}, async (session) => work(session as unknown as PostgresQueryable));
  }
  return withPostgresTransaction(database, work);
};
...
class RecurringRunRollbackError extends Error {
...
const buildRecurringDependencies = (
  database: RuntimeQueryTarget,
  scope: TenantScope,
): RecurringDomainDependencies => {
  const queryable = queryTarget(database);
  const clientRepository = () => createPostgresClientRepository(queryable);
  const invoiceRepository = () => createPostgresInvoiceRepository(queryable);
  const profileRepository = () => createPostgresRecurringProfileRepository(queryable);
  const read = () => readSettings(database, scope.tenantId);
  const transactionPort = {
    inTransaction: async <T>(work: () => Promise<T> | T): Promise<T> => {
      // processRecurringRun is wrapped by withRuntimeTransaction below. Nested
      // domain transactions must reuse that real session, never execute outside
      // it, otherwise numbering and invoice writes can partially commit.
      if (!recurringTransactionContext.getStore()) {
        throw new Error('Recurring transaction context is not available');
      }
      return work();
    },
  };

  const saveSettingsForScope = async (settings: RuntimeSettings, createdAt?: string): Promise<void> => {
  ...
  const numbering: DocumentNumberingPorts<RuntimeSettings> = {
    tx: transactionPort,
    getSettings: async () => (await read())?.settings ?? null,
    saveSettings: async (settings) => {
      const current = await read();
      await saveSettingsForScope(settings, current?.createdAt);
    },
    createReservation: async (reservation) => {
      await saveServerNumberReservation(queryable, {
        tenantId: scope.tenantId,
        ...reservation,
        createdAt: nowIso(),
        updatedAt: nowIso(),
    ...
    getReservationById: async (reservationId) => {
      const reservation = (await listServerNumberReservations(queryable, scope.tenantId))
    ...
    updateReservation: async (reservation) => {
      const existing = (await listServerNumberReservations(queryable, scope.tenantId))
    ...
    isNumberTaken: async (kind, number) => {
      ...
      const documents = kind === 'invoice'
        ...
        : await createPostgresOfferRepository(queryable).list(scope);
      if (documents.some((document) => document.number === number)) return true;
      const reservations = await listServerNumberReservations(queryable, scope.tenantId);
      return reservations.some((reservation) =>
        reservation.kind === kind && reservation.number === number && reservation.status !== 'released');
    },
    generateReservationId: async () => randomUUID(),
  };

  const projectPorts: DefaultProjectPorts<ClientProject> = {
    tx: transactionPort,
    getActiveDefaultProjectForClient: async (clientId) => {
      const client = await clientRepository().getById(scope, clientId);
      return client?.projects.find((project) =>
        project.name === 'Allgemein' && project.status === 'active' && !project.archivedAt) ?? null;
    },
    ...
    saveProject: async (project) => {
      ...
      const nextProject = { ...project, createdAt: project.createdAt ?? nowIso(), updatedAt: nowIso() };
  ...
  return {
    ...
    numberingPort: {
      getSettings: async () => (await read())?.settings ?? null,
      reserve: async (kind, at) => reserveDocumentNumber(numbering, kind, at),
      release: async (reservationId) => releaseDocumentNumber(numbering, reservationId),
      finalize: async (reservationId, documentId) => finalizeDocumentNumber(numbering, reservationId, documentId),
    },
    projectPort: {
      ensureDefaultProject: async (clientId) => (await ensureDefaultProjectForClient(projectPorts, {
        clientId,
...
export interface RegisterAutomationRoutesOptions {
  ...
  readonly requireDatabase: (app: FastifyInstance) => RuntimeDatabase;
}
...
export const registerAutomationRoutes = (
  ...
  typedRoute(app, {
    ...
    async handler({ request }) {
      ...
      const db = database();
      try {
        const result = await withRuntimeTransaction(db, async (queryable) =>
          recurringTransactionContext.run(queryable, async () => {
            const next = await processRecurringRun(session.scope, buildRecurringDependencies(queryable, session.scope));
            if (next.errors.length > 0) throw new RecurringRunRollbackError(next);
            await appendAudit(
              queryable,
              session.scope,
              session,
              'recurring.manual_run',
              'tenant',
        ...
        if (error instanceof RecurringRunRollbackError) {
          return { success: false, result: error.result, error: error.message };
        }
        return { success: false, error: error instanceof Error ? error.message : String(error) };
      }
    },
