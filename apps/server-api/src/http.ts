  ...
  app.setErrorHandler((error, request, reply) => {
    ...
    if (error instanceof Error) {
      ...
      const status =
        ...
        /^(POSTING_DATE_IN_CLOSED_PERIOD|BACKFILL_STALE_PREVIEW|BACKFILL_CONFIRMATION_HASH_MISMATCH|DATEV_EXPORT_CONTENT_UNAVAILABLE|DATEV_EXPORT_HASH_MISMATCH|DRAFT_NOT_APPROVED|DOCUMENT_HAS_ALLOCATIONS|DOCUMENT_NOT_POSTED|DOCUMENT_REVERSAL_REQUIRED|PAYMENT_REVERSAL_REQUIRED|ASSET_REVERSAL_REQUIRED|ASSET_CORRECTION_REQUIRED|VENDOR_HAS_POSTED_DOCUMENTS|PAYMENT_SOURCE_ALREADY_BOOKED|PERIOD_LOCKED|SOFT_LOCK_|FINALIZED_|ALREADY_|.*_POSTED$|.*_CLOSED$)/.test(code) ? 409 :
        ...
        /^(ACCOUNTING_AUDIT_REASON_REQUIRED|DATEV_EXPORT_CONTENT_REQUIRED|INVALID_|MISSING_|UNKNOWN_|EMPTY_|PAYMENT_|OPEN_ITEM_|DRAFT_|BANK_TRANSACTION_|BACKFILL_)/.test(code) ? 400 :
