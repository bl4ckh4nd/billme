...
export const DunningModal: React.FC<DunningModalProps> = ({
  ...
  return (
      <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-surface rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-scale-in">
              ...
              <div className="p-6 overflow-y-auto flex-1 space-y-3">
                   {overdueInvoices.map(inv => {
                       ...
                       return (
                           <div key={inv.id} className={`p-4 rounded-xl border-2 cursor-pointer transition-[background-color,border-color,color,box-shadow,transform,opacity] ease-out duration-150 ${isSelected ? 'border-foreground bg-surface-muted' : 'border-border-subtle bg-surface hover:border-border'}`}
                                onClick={() => {
                           ...
                           >
                               <div className="flex justify-between items-center mb-2">
                                   <div className="flex items-center gap-3">
                                        <div className={`w-5 h-5 rounded border flex items-center justify-center ${isSelected ? 'bg-foreground border-foreground text-white' : 'border-border'}`}>
