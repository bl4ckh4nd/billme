  ...
  return (
    <div className="max-w-2xl space-y-8 animate-enter">
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className={settings.legal.smallBusinessRule ? 'opacity-30 pointer-events-none' : ''}>
          ...
          <input
            ...
            className="w-full bg-surface-muted border border-border rounded-lg p-4 font-bold text-foreground tabular-nums focus:ring-2 focus:ring-accent outline-none transition-shadow duration-150 ease-out"
          />
        </div>
        <div>
      ...
      <div className="bg-surface border border-border-subtle rounded-xl p-6">
        ...
        <div className="flex items-center gap-2 bg-canvas/80 p-1.5 rounded-full border border-border w-fit">
          {(['soll', 'ist'] as const).map((m) => (
            <button
              key={m}
              onClick={() => updateNested('legal', 'taxAccountingMethod', m)}
              className={`px-5 py-2 rounded-full text-xs font-bold transition-[background-color,border-color,color,box-shadow,transform,opacity] ease-out ${
      ...
      <div className="border-t border-border-subtle pt-6">
        ...
        <div className="mb-6">
          ...
          <textarea
            ...
            rows={3}
            className="w-full bg-surface-muted border border-border rounded-lg p-4 text-sm font-medium focus:ring-2 focus:ring-accent outline-none resize-none transition-shadow duration-150 ease-out"
          />
        </div>
        <div>
