...
export const ShortcutsModal: React.FC<ShortcutsModalProps> = ({ open, onClose }) => {
  ...
  return (
    <Modal open={open} onClose={onClose} titleId={titleId} className="max-w-md overflow-hidden">
      <div className="p-6">
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 id={titleId} className="text-lg font-semibold text-foreground">Tastenkürzel</h2>
          </div>
          <button
            type="button"
              ...
              <div className="ml-4 flex shrink-0 items-center gap-1">
                {keys.map((k, i) => (
                  <React.Fragment key={k}>
                    ...
                    <kbd className="inline-flex min-w-[28px] items-center justify-center rounded-lg border border-border bg-border-subtle px-2 py-0.5 text-xs font-semibold text-foreground">
                      {k}
                    </kbd>
                  </React.Fragment>
                ))}
