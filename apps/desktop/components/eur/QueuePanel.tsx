...
export const QueuePanel: React.FC<Props> = ({
  ...
  const isFiltered = query.trim().length > 0 || flowFilter !== 'all';
    ...
    <div className="rounded-xl border border-border p-4 lg:col-span-1">
        ...
        <div className="grid grid-cols-2 gap-1 text-xs">
          {([
            ...
            <button
              ...
              className={`rounded-md px-2 py-1.5 font-medium transition-[background-color,border-color,color,box-shadow,opacity,transform,width] ease-out duration-150 ${
                queueStatus === status
      ...
      <div className="relative mb-3">
        ...
        <input
          ...
          className="w-full pl-9 pr-3 py-2 rounded-lg border border-border text-sm focus:outline-none focus:ring-2 focus:ring-foreground/10"
        />
      </div>
      ...
      <div className="mb-3 rounded-lg border border-border bg-surface-muted p-3">
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            ...
            className="text-xs text-muted hover:text-foreground transition-colors ease-out duration-150"
          >
            Auswahl löschen
          </button>
        </div>
        <div className="grid grid-cols-1 gap-1.5">
          <button
            ...
            className="flex items-center gap-2 rounded-lg border border-border bg-surface px-3 py-2 text-xs text-left hover:bg-canvas transition-colors ease-out duration-150 disabled:opacity-50"
          >
      ...
      {itemsLoading ? (
        ...
        isFiltered ? (
          <div className="flex flex-col items-center justify-center py-12 text-muted">
            ...
            <p className="text-lg font-medium">Kein Suchergebnis</p>
            <p className="text-sm text-center mt-2">Suche oder Filter anpassen.</p>
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center py-12 text-muted">
        ...
        <div className="space-y-2 max-h-[460px] overflow-auto pr-1">
          {queueItems.map((item, idx) => {
            ...
            return (
              <div
                key={key}
                className={`w-full text-left p-3 rounded-xl border transition-[background-color,border-color,color,box-shadow,opacity,transform,width] ease-out duration-150 animate-enter ${
                  isActive ? 'border-foreground bg-surface-muted shadow-sm' : 'border-border hover:border-muted/30 hover:shadow-sm'
                }`}
