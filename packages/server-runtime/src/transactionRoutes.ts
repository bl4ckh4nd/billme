...
import { requireDatabase, requireSession } from './app.js';
...
const requireMutation = async (app: FastifyInstance, product: 'lite' | 'pro', authorization: string | undefined) => {
  const session = await requireSession(app, product, authorization);
  if (!['owner', 'admin', 'accountant'].includes(session.role)) {
    throw new ApiError(403, 'Transaction mutation requires owner, admin, or accountant role');
  }
  return session;
};
...
export const registerTransactionRoutes = (app: FastifyInstance, product: 'lite' | 'pro'): void => {
  ...
  typedRoute(app, {
    ...
    async handler({ request, body }) {
      ...
      try {
        return await commitServerTransactionImport(requireDatabase(app), session.scope, body, {
          id: session.user.id,
  ...
  typedRoute(app, {
    ...
    async handler({ request, params, body }) {
      const session = await requireMutation(app, product, request.headers.authorization);
      try {
        return await rollbackServerImportBatch(requireDatabase(app), session.scope, params.batchId, body.reason, {
          id: session.user.id,
          displayName: session.user.fullName,
        });
      } catch (error) {
  ...
  typedRoute(app, {
    ...
    async handler({ request, params, body }) {
      const session = await requireMutation(app, product, request.headers.authorization);
      try {
        return await linkServerTransaction(requireDatabase(app), session.scope, params.transactionId, body.invoiceId, body.reason, {
          id: session.user.id,
          displayName: session.user.fullName,
        });
      } catch (error) {
  ...
  typedRoute(app, {
    ...
    async handler({ request, params, body }) {
      const session = await requireMutation(app, product, request.headers.authorization);
      try {
        return await unlinkServerTransaction(requireDatabase(app), session.scope, params.transactionId, body.reason, {
          id: session.user.id,
          displayName: session.user.fullName,
        });
      } catch (error) {
