...
import type { MatchingTab, EurStatus, EurTxItem, EurUndo, MatchResult } from './transaction-matching/types';
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2 rounded-lg border border-border p-1 bg-surface-muted">
            <button
              ...
              className={`px-3 py-1.5 text-sm rounded-md ${activeTab === 'matching' ? 'bg-surface shadow font-semibold text-foreground' : 'text-muted'}`}
            >
              Rechnungen zuordnen
            </button>
            <button
              onClick={() => setActiveTab('eur')}
              className={`px-3 py-1.5 text-sm rounded-md ${activeTab === 'eur' ? 'bg-surface shadow font-semibold text-foreground' : 'text-muted'}`}
            >
              EÜR klassifizieren
            </button>
          </div>
