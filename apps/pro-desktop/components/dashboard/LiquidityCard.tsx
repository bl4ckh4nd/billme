...
interface LiquidityCardProps {
  ...
  onSaveDueSoonDays: (days: number) => void;
    ...
    <div className="bg-dark-3 rounded-xl p-8 text-white relative overflow-hidden min-h-[420px] flex flex-col justify-between group shadow-xl animate-enter premium-hover">
       ...
       <div className="relative z-10">
           <div className="flex justify-between items-start mb-12">
               ...
               <div className="flex gap-2 items-center">
                   <DashboardSettingsPopover
                     ...
                     values={{ dueSoonDays }}
                     onSave={(v) => onSaveDueSoonDays(v.dueSoonDays)}
           ...
           <div className="mb-4">
               ...
               <h2 className="text-5xl tabular-nums font-bold tracking-tight mb-4">{formatCurrency(kpis.outstandingTotal)}</h2>
               ...
               <div className="flex flex-col gap-3">
                   <div className="flex items-center justify-between bg-white/5 p-3 rounded-xl border border-white/5 hover:bg-white/10 transition-colors">
                      <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm font-bold text-white">Überfällig ({kpis.overdueCount})</span>
                      </div>
                      <span className="tabular-nums font-bold text-error">{formatCurrency(kpis.overdueTotal)}</span>
                   </div>
                   <div className="flex items-center justify-between bg-white/5 p-3 rounded-xl border border-white/5 hover:bg-white/10 transition-colors">
                      <div className="flex items-center gap-3">
                          <div className="w-2 h-2 rounded-full bg-accent shadow-[0_0_10px_rgba(217,249,68,0.5)]"></div>
                          <span className="text-sm font-bold text-white">Fällig in {dueSoonDays} Tagen ({kpis.dueSoonCount})</span>
                      </div>
                      <span className="tabular-nums font-bold text-accent">{formatCurrency(kpis.dueSoonTotal)}</span>
                   </div>
               </div>
           </div>
       </div>
       ...
       <div className="relative z-10 pt-6 border-t border-white/10">
           <div className="flex justify-between items-end">
              ...
              <button
                  ...
                  className="bg-accent text-black px-6 py-3 rounded-xl font-bold text-sm hover:bg-accent-hover hover:scale-105 active:scale-95 transition-[background-color,border-color,color,box-shadow,transform,opacity] duration-200 ease-out shadow-lg shadow-accent/20"
              >
                  Mahnung senden
              </button>
           </div>
