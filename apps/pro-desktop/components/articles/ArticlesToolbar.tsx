    ...
    <div className="flex flex-col gap-6 mb-6 shrink-0">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted font-medium">{filteredCount} Einträge</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={onOpenForm}
            className="w-12 h-12 bg-foreground text-surface rounded-full flex items-center justify-center hover:scale-105 active:scale-95 transition-transform duration-150 ease-out shadow-sm ml-2"
          >
            <Plus size={24} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-x-auto scrollbar-hide flex gap-2 justify-end mask-linear-fade">
          {categories.map(cat => (
            <button
              ...
              className={`whitespace-nowrap px-4 py-2 rounded-full text-xs font-bold transition-[background-color,border-color,color,box-shadow,transform,opacity] duration-150 ease-out ${
      ...
      {selectedCount > 0 && (
        <div className="absolute top-8 left-1/2 -translate-x-1/2 bg-black text-white px-6 py-3 rounded-full shadow-2xl z-20 flex items-center gap-6 animate-in slide-in-from-top-4">
          <span className="text-sm font-bold">{selectedCount} ausgewählt</span>
          ...
          <button onClick={onClearSelection} className="ml-2 hover:text-dark-muted transition-colors duration-150 ease-out">
            <X size={16} />
          </button>
        </div>
      )}
