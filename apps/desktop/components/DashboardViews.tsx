...
import { Plus, ArrowRight, CreditCard, X } from 'lucide-react';
import { Account, AppSettings, Invoice } from '../types';
...
// Re-export TemplatesView so router.tsx keeps its unchanged import path.
export { TemplatesView } from './dashboard/TemplatesView';

// --- Shared helpers (kept here for the formatCurrency usage in AccountsView) ---
...
export const DashboardHome: React.FC<ViewProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-8">
      <LiquidityCard
        outstandingTotal={kpis.outstandingTotal}
        overdueCount={kpis.overdueCount}
        overdueTotal={kpis.overdueTotal}
        dueSoonCount={kpis.dueSoonCount}
        dueSoonTotal={kpis.dueSoonTotal}
      ...
      />
      <RevenueCard
        monthRevenueNet={kpis.monthRevenueNet}
        monthIssuedCount={kpis.monthIssuedCount}
      ...
      <PaymentsCard
        ...
        saveDashboardSettings={saveDashboardSettings}
      />
      <TaxEstimateCard taxEstimate={taxEstimate} taxMethod={taxMethod} />
    </div>
  );
};

// ---------------------------------------------------------------------------
// AccountsView (legacy export — behaviour-identical, uses AccountDetailView)
// ---------------------------------------------------------------------------
    ...
    if (selectedAccount) {
        return (
            <AccountDetailView
                ...
                onUpsertAccount={(acc) => upsertAccount.mutate(acc)}
        ...
        <div className="bg-surface rounded-xl shadow-sm overflow-hidden p-8 h-full animate-enter">
            <div className="flex items-center justify-between mb-8">
                ...
                <button
                  ...
                  className="bg-black text-white px-6 py-3 rounded-full text-sm font-bold hover:bg-dark-2 hover:scale-105 active:scale-95 transition-[background-color,border-color,color,box-shadow,opacity,transform,width] ease-out flex items-center gap-2 shadow-lg"
                >
                    <Plus size={16} />
                    Konto hinzufügen
                </button>
              ...
              <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4">
                <div className="bg-surface rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
                  <div className="p-6 border-b border-border-subtle flex justify-between items-center bg-surface-muted">
                    ...
                    <button onClick={() => setIsAddAccountOpen(false)} className="p-2 hover:bg-canvas rounded-full transition-colors ease-out"><X size={18} /></button>
                  </div>
                  ...
                  <div className="p-6 border-t border-border-subtle flex justify-end gap-2 bg-surface">
                    <button
                      ...
                      className="px-5 py-2.5 rounded-xl font-bold bg-canvas hover:bg-border transition-colors ease-out"
                    >
                      Abbrechen
                    </button>
                    <button
                      ...
                      className="px-5 py-2.5 rounded-xl font-bold bg-black text-white hover:bg-dark-2 transition-colors ease-out"
                    >
                      Speichern
                    </button>
                  </div>
             ...
             <div className="grid grid-cols-1 gap-4">
                {accounts.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-20 text-center gap-3">
                        <CreditCard size={40} className="text-muted opacity-40" />
                        <p className="text-sm font-bold text-foreground">Noch keine Konten vorhanden</p>
                        <p className="text-xs text-muted">Fügen Sie Ihr erstes Geschäftskonto hinzu.</p>
                    </div>
                )}
                {accounts.map((acc, i) => (
                    <div
                        ...
                        className={`p-6 rounded-xl ${acc.color} flex items-center justify-between cursor-pointer border border-transparent hover:border-black/10 hover:shadow-xl hover:-translate-y-1 transition-[background-color,border-color,color,box-shadow,opacity,transform,width] ease-out group animate-enter`}
                        style={{ animationDelay: `${i * 100}ms` }}
                    >
                        <div className="flex items-center gap-4">
                            <div className="w-14 h-14 bg-white rounded-xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform duration-200 ease-out">
                                <CreditCard size={24} className="text-foreground" />
                            </div>
                            <div>
                                <h4 className="font-bold text-lg group-hover:text-accent group-hover:bg-black group-hover:px-2 group-hover:rounded transition-[background-color,border-color,color,box-shadow,opacity,transform,width] ease-out">{acc.name}</h4>
                                <p className="text-sm text-muted font-mono mt-1">{acc.iban}</p>
                            </div>
                        </div>
                        <div className="text-right">
                            <p className="font-bold text-xl tabular-nums">{formatCurrency(acc.balance)}</p>
                            <span className="text-xs font-bold text-success bg-white px-2 py-1 rounded-full shadow-sm border border-success-border">Aktiv</span>
                            <div className="flex items-center justify-end gap-1 mt-2 text-xs font-bold text-muted group-hover:text-foreground tabular-nums">
