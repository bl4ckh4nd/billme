  ...
  <div className="grid grid-cols-12 gap-4 px-4 py-2 text-[10px] font-bold text-muted uppercase tracking-wider sticky top-0 bg-surface z-10 border-b border-border-subtle">
    <div className="col-span-1 flex justify-center">
      <button onClick={onSelectAll} className="hover:text-foreground">
        <CheckSquare size={16} className={hasSelection ? 'text-foreground fill-foreground/10' : 'text-muted'} />
      </button>
    </div>
...
export const ArticleListRow: React.FC<ArticleListRowProps> = ({
  ...
  return (
    <div
      className={`group rounded-xl p-4 border transition-[background-color,border-color,color,box-shadow,transform,opacity] duration-150 ease-out grid grid-cols-12 gap-4 items-center animate-enter ${
        isSelected
    ...
    >
      ...
      <div className="col-span-4 flex items-center gap-4">
        <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-xs font-bold shadow-sm shrink-0 ${getAvatarColor(article.category)}`}>
      ...
      <div className="col-span-1 text-center">
        <span className={`text-[10px] font-bold px-2 py-1 rounded ${article.taxRate === 19 ? 'bg-canvas text-muted' : 'bg-info-bg text-info'}`}>
          {article.taxRate}%
        </span>
      </div>
      ...
      <div className="col-span-2 flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-150 ease-out">
        <button onClick={() => onDuplicate(article)} className="p-2 bg-surface border border-border-subtle rounded-lg hover:bg-canvas text-muted transition-colors duration-150 ease-out"><Copy size={14}/></button>
        <button onClick={() => onEdit(article)} className="p-2 bg-surface border border-border-subtle rounded-lg hover:bg-foreground hover:text-surface transition-colors duration-150 ease-out"><Edit3 size={14}/></button>
        <button onClick={() => onDelete(article.id)} className="p-2 bg-surface border border-border-subtle text-error rounded-lg hover:bg-error hover:text-surface transition-colors duration-150 ease-out"><Trash2 size={14}/></button>
      </div>
    </div>
  );
};
