...
vi.mock('../db/proAccountingRepo', async () => {
  const actual = await vi.importActual<typeof import('../db/proAccountingRepo')>('../db/proAccountingRepo');
  return { ...actual, getAccountingPolicy: mockGetAccountingPolicy };
});
...
describe('Pro accounting IPC handlers', () => {
  ...
  it('registers and serves the typed authoritative policy route', async () => {
    ...
    mockGetAccountingPolicy.mockReturnValue(policy);
    ...
    expect(mockGetAccountingPolicy).toHaveBeenCalledWith(expect.anything(), 'default');
  });
});
