...
test('new accounting mutation boundaries require reason and idempotency', () => {
  ...
  assert.throws(() => closingCommandBodySchema.parse({ commandType: 'bad_debt', sourceId: 'settlement-1', sourceRevision: 'v1', idempotencyKey: 'bad-debt-key', input: {}, reason: 'Ausfall geprüft' }), /badDebtExpenseAccount/);
  assert.deepEqual(closingCommandBodySchema.parse({ commandType: 'advance_settlement', sourceId: 'settlement-1', sourceRevision: 'v1', idempotencyKey: 'advance-key', input: { advanceClearingReceivable: '1593', advanceClearingPayable: '1518' }, reason: 'Vorauszahlung geprüft' }).input, { advanceClearingReceivable: '1593', advanceClearingPayable: '1518' });
