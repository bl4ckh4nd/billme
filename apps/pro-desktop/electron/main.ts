...
let localBackend: ProLocalBackendHandle | null = null;
...
registerNativeIpcHandlers(ipcMain, {
  ...
  dumpDataDir: () => {
    if (!localBackend) throw new Error('Eingebettetes PGlite-Backend ist noch nicht initialisiert.');
...
app.whenReady().then(async () => {
  ...
  logger.error('Startup', 'Eingebettetes PGlite-Backend konnte nicht gestartet werden', startupError);
  try {
    await localBackend?.close();
  } catch (closeError) {
    logger.error(
      'Startup',
      'Eingebettetes PGlite-Backend konnte nach einem Startfehler nicht geschlossen werden',
...
app.on('before-quit', (event) => {
  ...
  void shutdownPromise.then(
    ...
    (error) => {
      logger.error(
        'Shutdown',
        'Eingebettetes PGlite-Backend konnte nicht geschlossen werden',
