        ...
        <div className="bg-surface rounded-xl shadow-sm p-8 min-h-[80vh] animate-enter">
            <div className="flex items-center justify-between mb-8">
                ...
                <div className="bg-canvas p-1 rounded-full flex items-center">
                    <button
                        ...
                        className={`px-6 py-2 rounded-full text-xs font-bold transition-[background-color,border-color,color,box-shadow,transform,opacity] duration-200 ease-out ${activeTab === 'invoice' ? 'bg-surface shadow text-foreground' : 'text-muted hover:text-foreground'}`}
                    >
                        Rechnungen
                    </button>
                    <button
                        onClick={() => setActiveTab('offer')}
                        className={`px-6 py-2 rounded-full text-xs font-bold transition-[background-color,border-color,color,box-shadow,transform,opacity] duration-200 ease-out ${activeTab === 'offer' ? 'bg-surface shadow text-foreground' : 'text-muted hover:text-foreground'}`}
                    >
                        Angebote
                    </button>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div
                    ...
                    className="aspect-[3/4] bg-surface-muted rounded-xl border-2 border-dashed border-border flex flex-col items-center justify-center gap-4 cursor-pointer hover:border-foreground hover:bg-canvas transition-[background-color,border-color,color,box-shadow,transform,opacity] duration-200 ease-out group animate-scale-in"
                >
                    <div className="w-16 h-16 bg-surface rounded-full flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform duration-200 ease-out">
                ...
                <div
                    key={t.id}
                    className="aspect-[3/4] bg-surface rounded-xl border border-border p-4 flex flex-col hover:border-border hover:-translate-y-2 transition-[background-color,border-color,color,box-shadow,transform,opacity] cursor-pointer group relative overflow-hidden animate-scale-in"
                ...
                >
                    <div className="flex-1 bg-surface-muted rounded-xl mb-4 relative overflow-hidden flex flex-col p-4 gap-2 border border-border-subtle">
                         ...
                         <div className="w-full h-1 bg-canvas rounded-full mt-2"></div>
                         <div className="w-full h-1 bg-canvas rounded-full"></div>

                         <div className="mt-auto bg-surface border border-border-subtle p-2 rounded-lg">
                             <div className="w-full h-1 bg-canvas rounded-full mb-1"></div>
                             <div className="flex justify-between">
                                 <div className="w-1/4 h-1 bg-canvas rounded-full"></div>
                    ...
                    <button
                        ...
                        className={`absolute top-4 left-4 px-3 py-1 rounded-full text-[10px] font-bold uppercase border transition-colors duration-200 ease-out ${
                    ...
                    <button onClick={(e) => { e.stopPropagation(); onOpenEditor(activeTab); }} className="absolute bottom-4 right-4 bg-foreground text-white w-10 h-10 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-[background-color,border-color,color,box-shadow,transform,opacity] duration-200 ease-out translate-y-2 group-hover:translate-y-0">
                        <ArrowUpRight size={18} />
                    </button>

                    <div className="absolute top-4 right-4">
                        {activeTab === 'offer' && (
                            <span className="bg-canvas text-muted px-2 py-1 rounded text-[10px] font-bold uppercase">Angebot</span>
                        )}
                    </div>
                </div>
                ))}
