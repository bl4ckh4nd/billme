...
import {
    Search, Plus, LayoutGrid, List, Users, SearchX
} from 'lucide-react';
...
export const ClientsView: React.FC = () => {
    ...
    return (
         <div className="bg-surface rounded-xl p-6 min-h-full border border-border flex flex-col animate-enter overflow-hidden">
              {isEditorOpen && draft ? (
                 ...
                 <div className="flex gap-3">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="pl-12 pr-6 py-3 bg-surface-muted border border-border rounded-full text-sm font-bold outline-none w-64 focus:ring-2 focus:ring-accent transition-shadow duration-150 ease-out"
                        />
                   </div>
                   ...
                   <div className="bg-canvas p-1 rounded-full flex items-center">
                        <button
                            ...
                            className={`w-10 h-10 rounded-full flex items-center justify-center transition-[background-color,border-color,color,box-shadow,transform,opacity] duration-150 ease-out ${viewMode === 'grid' ? 'bg-surface shadow text-foreground' : 'text-muted hover:text-foreground'}`}
                            title="Rasteransicht"
                        >
                            <LayoutGrid size={18} />
                        </button>
                        <button
                            ...
                            className={`w-10 h-10 rounded-full flex items-center justify-center transition-[background-color,border-color,color,box-shadow,transform,opacity] duration-150 ease-out ${viewMode === 'list' ? 'bg-surface shadow text-foreground' : 'text-muted hover:text-foreground'}`}
                            title="Listenansicht"
                        >
                            <List size={18} />
                        </button>
                   ...
                   <button
                     ...
                     className="w-12 h-12 bg-foreground text-white rounded-full flex items-center justify-center hover:scale-105 active:scale-95 transition-transform duration-150 ease-out shadow-lg"
                   >
                     <Plus size={24} />
                   </button>
                </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 overflow-y-auto pb-4">
                     {isLoading ? (
                       ...
                       <div className="col-span-full flex flex-col items-center justify-center py-24 text-center">
                           <Users size={48} className="text-muted opacity-30 mb-4" />
                           <h3 className="text-lg font-bold text-foreground mb-1">Noch keine Kunden angelegt</h3>
                           <p className="text-sm text-muted mb-6">Legen Sie Ihren ersten Kunden an, um Rechnungen und Angebote zu erstellen.</p>
                           <button
                               onClick={() => void openEditor()}
                               className="px-5 py-2.5 bg-foreground text-white rounded-xl text-sm font-bold hover:bg-dark-1 transition-colors duration-150 ease-out flex items-center gap-2"
                           >
                       ...
                       <div className="col-span-full flex flex-col items-center justify-center py-24 text-center">
                           <SearchX size={48} className="text-muted opacity-30 mb-4" />
                           <h3 className="text-lg font-bold text-foreground mb-1">Keine Treffer</h3>
                           <p className="text-sm text-muted mb-4">Keine Kunden für „{searchTerm}" gefunden.</p>
                           <button
                               onClick={() => setSearchTerm('')}
                               className="px-4 py-2 bg-surface border border-border text-foreground rounded-lg text-sm font-bold hover:bg-surface-muted transition-colors duration-150 ease-out"
                           >
                               Suche löschen
                           </button>
                       </div>
