...
import {
  ArrowLeft, Check,
  Edit3, Download, Mail, CheckCircle, Link, ExternalLink, RefreshCw,
  MoreHorizontal, Share2, Printer, ArrowRight, AlertTriangle, Clock,
} from 'lucide-react';
import { Badge, Button } from '@billme/ui';
import { Invoice, InvoiceItem, Payment, AppSettings } from '../../types';
      ...
      <div className="bg-surface rounded-xl p-8 min-h-full shadow-sm animate-enter relative">
          ...
          <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 mb-8 border-b border-border-subtle pb-8">
              <div className="flex items-start gap-4">
                  <button
                    onClick={onBack}
                    className="w-10 h-10 rounded-full border border-border flex items-center justify-center hover:bg-foreground hover:text-white transition-colors ease-out duration-150 shrink-0"
                  >
                      <ArrowLeft size={18} />
                  </button>
                  <div>
                       ...
                       <div className="flex items-center gap-3">
                            ...
                            {documentType === 'offer' && (
                                <span className="bg-accent/20 text-foreground px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider">Angebot</span>
                            )}
                       </div>
                  </div>
              </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
              ...
              <div className="space-y-6">
                  ...
                  <div className="bg-warning-bg border border-warning-border rounded-xl p-6">
                      ...
                      <textarea
                          className="w-full bg-surface border border-warning-border rounded-lg p-3 text-xs text-muted outline-none resize-none focus:ring-2 focus:ring-warning transition-shadow ease-out duration-150"
                          rows={3}
