      ...
      <div className="grid min-w-0 gap-5 xl:grid-cols-2">
          ...
          {!loading && feedback?.kind !== 'error' && items.length === 0 ? <p className="text-sm text-muted">Keine offenen Posten vorhanden.</p> : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                ...
                <tbody>{items.map((item) => { const itemId = `opos-open-item-${item.id}`; const itemError = paymentErrors[itemId]; return <tr key={item.id} className={`border-b border-border-subtle ${selectedItemId === item.id ? 'bg-surface-muted' : ''}`}><td className="p-2"><button id={itemId} type="button" aria-pressed={selectedItemId === item.id} aria-required="true" aria-invalid={itemError ? 'true' : undefined} aria-describedby={itemError ? `${itemId}-error` : undefined} className={`rounded-sm font-semibold underline-offset-2 hover:underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus-ring ${itemError ? 'border border-error text-error-text' : ''}`} onClick={() => selectOpenItem(item.id)}>{item.documentNumber}</button></td><td className="p-2">{item.partyType === 'debtor' ? 'Debitor' : 'Kreditor'}</td><td className="p-2 text-right">{euro(item.residualAmount)}</td><td className="p-2">{openItemStatusLabels[item.status]}</td></tr>; })}</tbody>
              </table>
            </div>
          )}
        ...
        <section className="min-w-0 rounded-xl border border-border p-4" aria-labelledby="incoming-heading">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="text-sm">Kreditor<select id={invoiceFieldIds.vendor} aria-label="Kreditor" className="mt-1 w-full rounded-lg border border-control-border px-3 py-2" value={vendorId} onChange={(e) => { setVendorId(e.target.value); clearInvoiceError(invoiceFieldIds.vendorName); }}><option value="">Neuen Kreditor anlegen</option>{vendors.map((vendor) => <option key={vendor.id} value={vendor.id}>{vendor.name}</option>)}</select></label>
            ...
            <label className="text-sm">Rechnungsdatum<input type="date" className="mt-1 w-full rounded-lg border border-control-border px-3 py-2" value={invoiceDate} onChange={(e) => setInvoiceDate(e.target.value)} /></label>
            <label className="text-sm">Fällig am<input type="date" className="mt-1 w-full rounded-lg border border-control-border px-3 py-2" value={dueDate} onChange={(e) => setDueDate(e.target.value)} /></label>
                ...
                {invoiceLines.map((line, index) => {
                  ...
                  return <div key={line.id} className="rounded-lg border border-border-subtle bg-surface-muted p-3">
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      ...
                      <label className="text-sm">Aufwandskonto<input aria-label="Konto" inputMode="numeric" className="mt-1 w-full rounded-lg border border-control-border px-3 py-2" value={line.accountNumber} onChange={(e) => updateInvoiceLine(line.id, 'accountNumber', e.target.value)} placeholder="z. B. 4900" /></label>
                      <label className="text-sm">Anlagekonto (optional)<input aria-label="Anlagekonto" inputMode="numeric" className="mt-1 w-full rounded-lg border border-control-border px-3 py-2" value={line.assetAccountNumber} onChange={(e) => updateInvoiceLine(line.id, 'assetAccountNumber', e.target.value)} placeholder="z. B. 0480" /></label>
                    </div>
          ...
          {softLockOverride ? <label className="mt-2 block text-sm">Override-Begründung<input disabled={!canMutate} className="mt-1 w-full rounded-lg border border-control-border px-3 py-2" value={overrideReason} onChange={(e) => setOverrideReason(e.target.value)} /></label> : null}
