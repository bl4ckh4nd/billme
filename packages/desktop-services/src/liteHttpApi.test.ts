...
test('Lite HTTP adapter sends EÜR ranges as encoded query parameters without GET bodies', async () => {
  const requests: Array<{ url: string; method: string; body: unknown }> = [];
  const report = {
    taxYear: 2025,
    from: '2025-01-01',
    to: '2025-12-31',
    rows: [{ id: 'line-1', label: 'Betriebseinnahmen', kind: 'income' as const, exportable: true, sortOrder: 0, total: 42 }],
    summary: { incomeTotal: 42, expenseTotal: 0, surplus: 42 },
    unclassifiedCount: 0,
    warnings: [],
    catalog: { id: 'catalog-2025', version: '2025', sourceHash: 'a'.repeat(64), delivery: 'print-form-only' as const, elsterReady: false },
  };
  const item = {
  ...
  const api = createLiteHttpBillmeApi({
    ...
    fetch: async (input, init) => {
      ...
      return response(url.includes('/reports/eur/items') ? [item] : report);
    },
  });

  await api.eur.getReport({ taxYear: 2025 });
