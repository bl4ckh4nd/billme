...
import type {
  ...
  BookingDraftEntity, JournalEntryEntity, JournalLineEntity, LedgerBalance, ValidationIssue, AccountingMutationContext,
...
const backfillConfig = async (db: PostgresQueryable, t: string): Promise<unknown> => { const p = await policy(db, t); const rows = await q<any>(db, `SELECT chart,role,account_number FROM accounting_account_mappings WHERE tenant_id=$1 ORDER BY chart,role`, [t]); return { policy: p, mappings: rows }; };
...
export const createPostgresProAccountingRepository = (db: PostgresQueryable): ProAccountingRepository => ({
  ...
  async listDatevExports(scope) { return (await q<any>(db, `SELECT * FROM datev_exports WHERE tenant_id=$1 ORDER BY created_at DESC`, [tenant(scope)])).map((r) => ({ id: r.id, filePath: r.file_path, recordCount: Number(r.record_count), fromDate: r.from_date ?? undefined, toDate: r.to_date ?? undefined, createdAt: r.created_at })); },
  async insertDatevExport(scope, args) { return inTx(db, async (tx) => { const existing = (await q<any>(tx, `SELECT * FROM datev_exports WHERE tenant_id=$1 AND file_path=$2`, [tenant(scope), args.filePath]))[0]; if (existing) return { id: existing.id, filePath: existing.file_path, recordCount: Number(existing.record_count), fromDate: existing.from_date ?? undefined, toDate: existing.to_date ?? undefined, createdAt: existing.created_at }; const id = randomUUID(); const createdAt = now(); const metadata = { immutable: true, contentSha256: args.contentSha256, sourceSnapshot: args.sourceSnapshot }; await q(tx, `INSERT INTO datev_exports (id,tenant_id,file_path,record_count,from_date,to_date,created_at,meta_json) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`, [id, tenant(scope), args.filePath, args.recordCount, args.fromDate ?? null, args.toDate ?? null, createdAt, JSON.stringify(metadata)]); await audit(tx as PostgresTransactionClient, scope, 'pro_datev_export', id, 'export', 'DATEV export recorded', null, { ...args, mutation: undefined }, args.mutation); return { id, filePath: args.filePath, recordCount: args.recordCount, fromDate: args.fromDate, toDate: args.toDate, createdAt }; }); },
...
const postDocument = async (db: PostgresQueryable, scope: TenantScope, type: 'outgoing_invoice'|'incoming_invoice', id: string, options: { softLockOverride?: boolean; overrideReason?: string; reservationId?: string; mutation?: AccountingMutationContext }, allowUnfinalizedReservation = false): Promise<AccountingPostingPreview> => {
...
const allocatePayment = async (db: PostgresQueryable, scope: TenantScope, input: OpenItemPaymentInput): Promise<OpenItemPaymentEntity> => {
  const t = tenant(scope);
  const stamp = now();
  const allocationEventId = input.allocationEventId?.trim() || randomUUID();
