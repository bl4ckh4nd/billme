...
const openRouterVlmEvidenceBoxSchema = z.object({
  x: z.number().min(0).max(1), y: z.number().min(0).max(1), width: z.number().min(0).max(1), height: z.number().min(0).max(1),
});
export const openRouterVlmEvidenceSchema = z.object({
  field: z.string().trim().min(1).max(100), value: z.string().max(1000), page: z.number().int().positive().optional(), boundingBox: openRouterVlmEvidenceBoxSchema.optional(), quote: z.string().max(1000).optional(), confidence: z.number().min(0).max(1),
});
const openRouterVlmNullableNumberSchema = z.number().finite().nullable();
const openRouterVlmNullableStringSchema = z.string().max(1000).nullable();
export const openRouterVlmExtractionSchema = z.object({
  ...
  issuer: openRouterVlmNullableStringSchema, recipient: openRouterVlmNullableStringSchema, invoiceNumber: openRouterVlmNullableStringSchema, invoiceDate: openRouterVlmNullableStringSchema, servicePeriod: openRouterVlmNullableStringSchema, dueDate: openRouterVlmNullableStringSchema, currency: z.string().length(3).nullable(),
  netAmount: openRouterVlmNullableNumberSchema, taxAmount: openRouterVlmNullableNumberSchema, grossAmount: openRouterVlmNullableNumberSchema,
  vatBreakdown: z.array(z.object({ rate: z.number().min(0).max(100), netAmount: z.number().finite(), taxAmount: z.number().finite() })).max(20),
  iban: openRouterVlmNullableStringSchema, paymentReference: openRouterVlmNullableStringSchema, suggestedAccountNumber: openRouterVlmNullableStringSchema, suggestedTaxCase: openRouterVlmNullableStringSchema,
  matchAssessment: z.object({ amountMatches: z.boolean().nullable(), dateMatches: z.boolean().nullable(), partyMatches: z.boolean().nullable(), referenceMatches: z.boolean().nullable(), notes: z.array(z.string().max(500)).max(20) }),
  warnings: z.array(z.string().max(500)).max(30), evidence: z.array(openRouterVlmEvidenceSchema).max(100),
});
export const openRouterVlmAnalyzeInputSchema = z.object({ transaction: openRouterVlmTransactionSchema, document: openRouterVlmDocumentSchema, model: z.string().trim().min(1).max(200).optional() });
export const openRouterVlmAnalyzeResultSchema = z.object({
  extraction: openRouterVlmExtractionSchema,
  deterministicChecks: z.object({ amountMatches: z.boolean(), currencyMatches: z.boolean(), expectedAmount: z.number().finite(), extractedAmount: z.number().finite().nullable(), expectedCurrency: z.string().length(3), extractedCurrency: z.string().length(3).nullable() }),
  metadata: z.object({
    model: z.string().min(1), provider: z.string().nullable(), requestId: z.string().nullable(), request: z.object({ method: z.literal('POST'), endpoint: z.string().url() }), timing: z.object({ startedAt: z.string().datetime(), completedAt: z.string().datetime(), durationMs: z.number().int().nonnegative() }), documentSha256: z.string().length(64),
  }),
});
export const openRouterVlmConfigSchema = z.object({ configured: z.boolean(), model: z.string().min(1), models: z.array(z.string().min(1)), maxDocumentBytes: z.number().int().positive(), timeoutMs: z.number().int().positive() });

/** Text artifacts generated by the server/PGlite tax export service. */
