...
export const ArticleGridCard: React.FC<ArticleGridCardProps> = ({
  ...
  return (
    <div
      className="group bg-surface-muted rounded-xl p-6 border border-border-subtle hover:border-border hover:bg-surface hover:-translate-y-1 transition-[background-color,border-color,color,box-shadow,opacity,transform,width] duration-150 ease-out relative flex flex-col animate-scale-in"
      style={{ animationDelay: `${idx * 50}ms` }}
    >
      <div className="flex justify-between items-start mb-4">
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-sm font-bold shadow-sm transition-transform duration-150 ease-out group-hover:scale-110 ${getAvatarColor(article.category)}`}>
        ...
        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150 ease-out">
          ...
          <button
            ...
            className="p-2 bg-surface rounded-lg hover:bg-foreground hover:text-white transition-colors duration-150 ease-out"
            title="Bearbeiten"
          >
            <Edit3 size={14}/>
          </button>
