...
if (process.env.BILLME_E2E) console.log('[e2e-startup] desktop main loaded');
...
app.whenReady().then(async () => {
  if (process.env.BILLME_E2E) console.log('[e2e-startup] desktop app ready');
  ...
  if (process.env.BILLME_E2E) console.log('[e2e-startup] desktop database ready');
  localAgentBridge = await startLocalAgentBridge({
    userDataPath,
    product: 'lite',
    invoke: ({ action, args }) => ipcHandlers.invoke(action as IpcRouteKey, args as never),
  });
  if (process.env.BILLME_E2E) console.log('[e2e-startup] desktop agent bridge ready');
  ...
  if (process.env.BILLME_E2E) console.log('[e2e-startup] desktop creating window');
  await createWindow();
