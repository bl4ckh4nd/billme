...
import { z } from 'zod';
import { requireDatabase, requireMutationSessionFor, requireSession, toMutationActor } from './runtimeContext.js';
...
const actorFor = (session: Awaited<ReturnType<typeof requireSession>>) => toMutationActor(session);
...
export const registerEurRuleRoutes = (app: FastifyInstance, product: 'lite' | 'pro'): void => {
  ...
  typedRoute(app, {
    ...
    async handler({ request, query }) {
      ...
      const rules = await listServerEurRules(requireDatabase(app), session.scope, query.taxYear);
  ...
  typedRoute(app, {
    ...
    async handler({ request, body }) {
      const session = await requireMutationSessionFor(app, product, request.headers.authorization, 'EÜR-Regeln benötigen die Rolle owner, admin oder accountant.');
      const { reason, ...ruleInput } = body;
      const saved = await upsertServerEurRule(requireDatabase(app), session.scope, ruleInput);
      await createPostgresBillingDependencies(requireDatabase(app)).auditLog.append(session.scope, {
  ...
  typedRoute(app, {
    ...
    async handler({ request, params, body }) {
      const session = await requireMutationSessionFor(app, product, request.headers.authorization, 'EÜR-Regeln benötigen die Rolle owner, admin oder accountant.');
      const deleted = await deleteServerEurRule(requireDatabase(app), session.scope, params.id);
