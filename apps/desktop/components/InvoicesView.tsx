...
import {
  ...
  Check,
  ChevronDown,
  Gavel, RefreshCw, LayoutTemplate
} from 'lucide-react';
...
import { getDisplayLineTotal } from './invoices/helpers';
...
export const DocumentsView: React.FC<DocumentsViewProps> = ({
  ...
  const selectedDocumentItems = selectedDocument
    ...
    : [];
  const recalculatedSelectedDocumentTax = selectedDocument
    ? calculateInvoiceTaxSnapshot(
        {
          items: selectedDocumentItems,
  ...
  const selectedDocumentTax =
    selectedDocument
      ? (selectedDocument.taxSnapshot &&
        recalculatedSelectedDocumentTax &&
        Math.abs((selectedDocument.taxSnapshot.netAmount ?? 0) - recalculatedSelectedDocumentTax.netAmount) < 0.005
          ? selectedDocument.taxSnapshot
          : recalculatedSelectedDocumentTax)
      : null;
  ...
  const handleProcessDunningRun = async () => {
      ...
      const selectedIdsSnapshot = [...selectedForDunning];
      ...
      for (const invoiceId of selectedIdsSnapshot) {
  ...
  const handleCreateReminder = () => {
      ...
      if (confirm(`${levelConfig?.name} erstellen für ${selectedDocument.number}?\nGebühr: ${new Intl.NumberFormat('de-DE', { style: 'currency', currency: 'EUR' }).format(levelConfig?.fee || 0)}`)) {
          if (documentType === 'invoice') {
              const historyEntry = {
                  date: new Date().toISOString().split('T')[0],
                  action: `${levelConfig?.name} erstellt (+${new Intl.NumberFormat('de-DE', { style: 'currency', currency: 'EUR' }).format(levelConfig?.fee || 0)})`
              };
              upsertInvoice.mutate({
                invoice: {
                  ...selectedDocument,
  ...
  // --- Payment Handlers ---
  const handlePaymentSave = (updatedInvoice: Invoice, reason: string) => {
    upsertInvoice.mutate(
  ...
  const handlePaymentDeleteSave = (updatedInvoice: Invoice, reason: string) => {
    upsertInvoice.mutate(
      { invoice: updatedInvoice, reason },
      {
        onSuccess: () => {
  ...
  const handleEditPayment = (payment: Invoice['payments'][number]) => {
  ...
  if (viewMode === 'detail' && selectedDocument) {
    return (
      <DetailView
        ...
        selectedTaxDefinition={selectedTaxDefinition}
        selectedTaxExemptionReason={selectedTaxExemptionReason}
        ...
        onOpenPath={(path) => void ipc.shell.openPath({ path })}
        ...
        paymentDeleteModal={
          selectedDocument ? (
            <PaymentDeleteModal
              ...
              selectedDocument={selectedDocument}
              onSave={handlePaymentDeleteSave}
            />
          ) : null
        }
        convertModal={
        ...
        <div className="flex items-center gap-4">
           ...
           <div className="relative">
                ...
                >
                    ...
                    <ChevronDown size={28} className={`transition-transform duration-200 ease-out ${isTypeDropdownOpen ? 'rotate-180' : ''}`} />
                </button>

                {isTypeDropdownOpen && (
                    <div className="absolute top-full left-0 mt-2 w-56 bg-surface rounded-2xl shadow-xl border border-border-subtle p-2 z-50 animate-in fade-in zoom-in-95 duration-200 ease-out">
                        <button
                            onClick={() => switchDocumentType('invoice')}
                            className={`w-full text-left px-4 py-3 rounded-xl font-bold text-sm transition-colors ease-out duration-150 flex items-center justify-between ${documentType === 'invoice' ? 'bg-foreground text-surface' : 'hover:bg-surface-muted text-foreground'}`}
                        >
                            Rechnungen
                        ...
                        <button
                            ...
                            className={`w-full text-left px-4 py-3 rounded-xl font-bold text-sm transition-colors ease-out duration-150 flex items-center justify-between ${documentType === 'offer' ? 'bg-foreground text-surface' : 'hover:bg-surface-muted text-foreground'}`}
                        >
                            Angebote
                ...
                {(['all', 'open', 'paid', 'overdue'] as const).map(s => {
                    ...
                    return (
                        <button
                            key={s}
                            onClick={() => setFilter(s)}
                            className={`px-4 py-1.5 rounded-full text-xs font-bold transition-colors ease-out duration-150 ${filter === s ? 'bg-foreground text-surface shadow-lg' : 'bg-canvas text-muted hover:bg-surface-muted'}`}
                        >
                            {labels[s]}
                        </button>
                    );
        ...
        <div className="flex gap-3 items-center">
           ...
           <div className="relative">
                ...
                <input
                    ...
                    className="pl-12 pr-6 py-3 bg-surface-muted border-none rounded-full text-sm font-bold outline-none w-64 focus:ring-2 focus:ring-accent transition-[background-color,border-color,color,box-shadow,opacity,transform,width] ease-out duration-150"
                />
           </div>
           ...
           <button
             ...
             className="w-12 h-12 bg-accent text-accent-foreground rounded-full flex items-center justify-center hover:scale-105 active:scale-95 transition-[background-color,border-color,color,box-shadow,opacity,transform,width] shadow-lg hover:bg-accent-hover"
             ...
             <SkeletonLoader variant="list" count={5} />
               ...
               <div className="flex flex-col items-center justify-center h-64 text-muted">
                   ...
                   {searchTerm || filter !== 'all' ? (
                     <>
                       <p className="font-bold text-foreground">Keine Treffer</p>
                       <p className="text-sm mt-1 text-muted">Keine Dokumente für die aktuelle Suche oder Filterung.</p>
                       <button
                         onClick={() => { setSearchTerm(''); setFilter('all'); }}
                         className="mt-4 px-4 py-2 rounded-lg bg-canvas text-foreground font-bold text-sm hover:bg-surface-muted transition-colors ease-out duration-150"
                       >
                         Filter zurücksetzen
                       </button>
                     </>
                     ...
                     <>
                       ...
                       <p className="text-sm mt-1 text-muted">
                         {documentType === 'invoice' ? 'Erstelle deine erste Rechnung.' : 'Erstelle dein erstes Angebot.'}
                       </p>
                       <button
                         onClick={() => onCreateInvoice(documentType)}
                         className="mt-4 px-4 py-2 rounded-lg bg-accent text-accent-foreground font-bold text-sm hover:bg-accent-hover transition-colors ease-out duration-150 flex items-center gap-2"
                       >
                         <Plus size={16} />
                         {documentType === 'invoice' ? 'Rechnung erstellen' : 'Angebot erstellen'}
                       </button>
                     </>
                   )}
               </div>
